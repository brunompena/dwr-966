...
define interaptiv_check
	([ "$(1)" = "$(LINUX_DIR)" -a "$(2)" = "platform/" ] && [ ! -f "$(TOPDIR)/NoInterAptiv" ])
endef
...
define PatchDir/Quilt
	...
	@for patch in $$$$( (cd "$(2)" && if [ -f series ]; then \
		$(call interaptiv_check,$(1),$(3)) && ( ls *.patch | sort ) || $(call filter_series,series); \
		else ls | sort; fi; ) 2>/dev/null ); do ( \
		$(call interaptiv_check,$(1),$(3)) && { \
			cp "$(2)/$$$$patch" "$(1)/patches/$(3)"; \
			echo "$(3)$$$$patch" >> "$(1)/patches/series"; \
		} || { \
			echo "$(2)/$$$$patch" | grep -qi InterAptiv || { \
				cp "$(2)/$$$$patch" "$(1)/patches/$(3)"; \
				echo "$(3)$$$$patch" >> "$(1)/patches/series"; \
			} && true; \
		} \

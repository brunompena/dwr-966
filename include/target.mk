...
ifeq ($(DUMP),)
  ...
  PROFTARGET:=$(strip $(foreach subdir,$(patsubst $(PLATFORM_DIR)/$(SUBTARGET)/%/config-default,%,$(wildcard $(PLATFORM_DIR)/$(SUBTARGET)/*/config-default)),$(if $(CONFIG_TARGET_$(call target_conf,$(BOARD)_$(SUBTARGET)_$(subdir))),$(subdir))))
else
  PLATFORM_DIR:=${CURDIR}
  ifeq ($(SUBTARGETS),)
...
ifneq ($(PLATFORM_DIR),$(PLATFORM_SUBDIR))
  ifneq	($(PROFTARGET),)
     LINUX_SUBTARGET_CONFIG = $(call find_kernel_config,$(PLATFORM_SUBDIR)/$(PROFTARGET))
  else

...
#define SM(_v, _f)  (((_v) << _f##_S) & (_f))
	...
	&& defined(CONFIG_LANTIQ_PCIE_2ND_CORE)\
	&& defined(CONFIG_LANTIQ_PCIE_3RD_CORE)
#define LTQ_PCIE_PORT_MAX    3
#elif (defined(CONFIG_LANTIQ_PCIE_1ST_CORE) \
	&& defined(CONFIG_LANTIQ_PCIE_2ND_CORE)) \
	|| (defined(CONFIG_LANTIQ_PCIE_2ND_CORE)\
	&& defined(CONFIG_LANTIQ_PCIE_3RD_CORE)) \
	|| (defined(CONFIG_LANTIQ_PCIE_1ST_CORE) \
	&& defined(CONFIG_LANTIQ_PCIE_3RD_CORE))
#define LTQ_PCIE_PORT_MAX    2
#elif defined(CONFIG_LANTIQ_PCIE_1ST_CORE)\
	|| defined(CONFIG_LANTIQ_PCIE_2ND_CORE)\
	|| defined(CONFIG_LANTIQ_PCIE_3RD_CORE)
#define LTQ_PCIE_PORT_MAX    1
#else
#error "XRX200/300 not select any PCIe core!!!!"
#endif
...
extern void pcie_debug(const char *fmt, ...);
...
extern int pcie_rc_port_to_pcie_port(int rc_port, int *pcie_port);
extern int pcie_port_idx_to_rc_port(int pcie_port);
...
extern struct ltq_pcie_port g_pcie_port_defs[LTQ_PCIE_PORT_MAX];
...
#define PCIE_RC0_LED_RST	180
#define PCIE_RC1_LED_RST	181
...
static inline void pcie_ep_rst_init(int rc_port)
{
	if (of_machine_is_compatible("lantiq,vr9"))
		pcie_port_to_rst_pin[0] = PCIE_GPIO_RESET;

	gpio_request(pcie_port_to_rst_pin[rc_port], "pcie-reset");
	gpio_direction_output(pcie_port_to_rst_pin[rc_port], 1);
	if (rc_port == LTQ_PCIE_PORT0) {
	#ifdef CONFIG_LANTIQ_PCIE_RST_EP_ACTIVE_HIGH
		gpio_set_value(pcie_port_to_rst_pin[rc_port], 0);
	#else
		gpio_set_value(pcie_port_to_rst_pin[rc_port], 1);
	#endif
	} else if (rc_port == LTQ_PCIE_PORT1) {
	#ifdef CONFIG_LANTIQ_PCIE1_RST_EP_ACTIVE_HIGH
		gpio_set_value(pcie_port_to_rst_pin[rc_port], 0);
	#else
		gpio_set_value(pcie_port_to_rst_pin[rc_port], 1);
	#endif
	} else if (rc_port == LTQ_PCIE_PORT2) {
	#ifdef CONFIG_LANTIQ_PCIE2_RST_EP_ACTIVE_HIGH
		gpio_set_value(pcie_port_to_rst_pin[rc_port], 0);
	#else
		gpio_set_value(pcie_port_to_rst_pin[rc_port], 1);
	#endif
...
{
	u32 reg = ltq_r32(LTQ_FUSE_ID_CFG);
	if (rc_port == LTQ_PCIE_PORT0)
		return (reg & BIT(10)) ? 1 : 0;
	else if (rc_port == LTQ_PCIE_PORT1)
		return (reg & BIT(9)) ? 1 : 0;
	else if (rc_port == LTQ_PCIE_PORT2)
		return (reg & BIT(16)) ? 1 : 0;
	else
...
static inline void pcie_ahb_arbiter_setup(void)
{
	if (of_machine_is_compatible("lantiq,grx390")) {
		u32 set = 0;
		u32 clr = RCU_PCIE_ARBITER_MASK;
		u32 chipid = ltq_get_cpu_id();

		switch (chipid) {
		case SOC_ID_GRX383:
			set |= RCU_PCIE_ARBITER_RC0;
			break;
		case SOC_ID_GRX369:
		case SOC_ID_GRX389:
			set |= RCU_PCIE_ARBITER_RC0_RC1_RC2;
			break;
		case SOC_ID_GRX387:
			set |= RCU_PCIE_ARBITER_RC0_RC1;
			break;
		default:
			set |= RCU_PCIE_ARBITER_RC0_RC1;
			break;
		}
		ltq_rcu_w32_mask(clr, set, RCU_RST_REQ2);
	}
}
...
static inline void pcie_rcu_endian_setup(int rc_port)
{
	...
	if (of_machine_is_compatible("lantiq,vr9")) {
		...
		set |= RCU_VR9_BE_PCIE0S;
	#else
		clr |= RCU_VR9_BE_PCIE0S;
	#endif /* CONFIG_LANTIQ_PCIE_HW_SWAP */
		/* Shared with USIF, don't touch */
		clr |= RCU_VR9_BE_AHB2M;

		/* Inbound confiugration */
	#ifdef CONFIG_LANTIQ_PCIE_INBOUND_NO_HW_SWAP
		clr |= RCU_VR9_BE_PCIE0M;
	#else
		set |= RCU_VR9_BE_PCIE0M;
	#endif /* CONFIG_LANTIQ_PCIE_INBOUND_NO_HW_SWAP */
	} else {
	if (of_machine_is_compatible("lantiq,grx390")) {
#if defined(CONFIG_LANTIQ_PCIE_INBOUND_NO_HW_SWAP) \
		&& defined(CONFIG_LANTIQ_PCIE1_INBOUND_NO_HW_SWAP) \
		&& defined(CONFIG_LANTIQ_PCIE2_INBOUND_NO_HW_SWAP)
		...
		/*
		 * Outbound, common setting is little endian,
		 * PCI_S can be controlled
		 * seperatelyaccording to sw swap requirement.
		 */
		clr |= RCU_BE_AHB3M;
		if (rc_port == LTQ_PCIE_PORT0) {
		#ifdef CONFIG_LANTIQ_PCIE_INBOUND_NO_HW_SWAP
			...
			set |= RCU_BE_PCIE0S;
		#else
			/* Outbound converteded to little endian,
			 * no swap needed */
			clr |= RCU_BE_PCIE0S;
		#endif
		} else if (rc_port == LTQ_PCIE_PORT1) {
		...
		#ifdef CONFIG_LANTIQ_PCIE2_INBOUND_NO_HW_SWAP
			clr |= RCU_BE_PCIE2M;
		#else
			set |= RCU_BE_PCIE2M;
	...
	ltq_rcu_w32_mask(clr, set, RCU_AHB_ENDIAN);
	pcie_dbg(PCIE_MSG_REG,
		"%s port %d IFX_RCU_AHB_ENDIAN: 0x%08x\n",
		__func__, rc_port, ltq_rcu_r32(RCU_AHB_ENDIAN));
}
...
static inline void pcie_phy_pmu_enable(int rc_port)
{
	...
	if (rc_port == LTQ_PCIE_PORT0)/* XXX, should use macro */
		clk = clk_get_sys("1d900000.pcie", "phy");
	else if (rc_port == LTQ_PCIE_PORT1)
		clk = clk_get_sys("19000000.pcie", "phy");
	else if (rc_port == LTQ_PCIE_PORT2)
		clk = clk_get_sys("1a800000.pcie", "phy");
	else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
...
static inline void pcie_pdi_big_endian(int rc_port)
{
	...
	if (rc_port == LTQ_PCIE_PORT0) {
		...
		set |= RCU_BE_PCIE0_PDI;
	} else if (rc_port == LTQ_PCIE_PORT1) {
		/* Config AHB->PCIe and PDI endianness */
		set |= RCU_BE_PCIE1_PDI;
	} else if (rc_port == LTQ_PCIE_PORT2) {
		/* Config AHB->PCIe and PDI endianness */
		set |= RCU_BE_PCIE2_PDI;
	} else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
	ltq_rcu_w32_mask(0, set, RCU_AHB_ENDIAN);
}
...
static inline void pcie_pdi_pmu_enable(int rc_port)
{
	...
	if (rc_port == LTQ_PCIE_PORT0)
		...
		clk = clk_get_sys("19000000.pcie", "pdi");
	else if (rc_port == LTQ_PCIE_PORT2)
		clk = clk_get_sys("1a800000.pcie", "pdi");
	else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
	if (!IS_ERR(clk))
		clk_enable(clk);
...
static inline void pcie_core_rst_assert(int rc_port)
{
	/* Reset Core, bit 22 */
	if (rc_port == LTQ_PCIE_PORT0)
		ltq_rcu_w32_mask(0, BIT(22), RCU_RST_REQ);
	else if (rc_port == LTQ_PCIE_PORT1)
		ltq_rcu_w32_mask(0, BIT(27), RCU_RST_REQ);
	else if (rc_port == LTQ_PCIE_PORT2)
		ltq_rcu_w32_mask(0, BIT(28), RCU_RST_REQ2);
	else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
}
...
static inline void pcie_core_rst_deassert(int rc_port)
{
	...
	if (rc_port == LTQ_PCIE_PORT0)
		ltq_rcu_w32_mask(BIT(22), 0, RCU_RST_REQ);
	else if (rc_port == LTQ_PCIE_PORT1)
		ltq_rcu_w32_mask(BIT(27), 0, RCU_RST_REQ);
	else if (rc_port == LTQ_PCIE_PORT2)
		ltq_rcu_w32_mask(BIT(28), 0, RCU_RST_REQ2);
	else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
}
...
{
	if (rc_port == LTQ_PCIE_PORT0)
		ltq_rcu_w32_mask(0, BIT(12), RCU_RST_REQ);
	else if (rc_port == LTQ_PCIE_PORT1)
		ltq_rcu_w32_mask(0, BIT(13), RCU_RST_REQ);
	else if (rc_port == LTQ_PCIE_PORT2)
		ltq_rcu_w32_mask(0, BIT(29), RCU_RST_REQ2);
	else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
}
...
static inline void pcie_phy_rst_deassert(int rc_port)
{
	...
	if (rc_port == LTQ_PCIE_PORT0)
		ltq_rcu_w32_mask(BIT(12), 0, RCU_RST_REQ);
	else if (rc_port == LTQ_PCIE_PORT1)
		ltq_rcu_w32_mask(BIT(13), 0, RCU_RST_REQ);
	else if (rc_port == LTQ_PCIE_PORT2)
		ltq_rcu_w32_mask(BIT(29), 0, RCU_RST_REQ2);
	else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
}
...
static inline void pcie_core_pmu_setup(int rc_port)
{
	...
	if (rc_port == LTQ_PCIE_PORT0) {
		...
		clk = clk_get_sys("19000000.pcie", "ctl");
	else if (rc_port == LTQ_PCIE_PORT2)
		clk = clk_get_sys("1a800000.pcie", "ctl");
	else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
	if (!IS_ERR(clk))
		clk_enable(clk);
...
static inline void pcie_msi_init(int pcie_port)
{
	...
	if (rc_port == LTQ_PCIE_PORT0)
		clk = clk_get_sys("1d900000.pcie", "msi");
	else if (rc_port == LTQ_PCIE_PORT1)
		clk = clk_get_sys("19000000.pcie", "msi");
	else if (rc_port == LTQ_PCIE_PORT2)
		clk = clk_get_sys("1a800000.pcie", "msi");
	else
		PCIE_KASSERT(0, ("%s invalid port %d\n", __func__, rc_port));
	if (!IS_ERR(clk))
		clk_enable(clk);

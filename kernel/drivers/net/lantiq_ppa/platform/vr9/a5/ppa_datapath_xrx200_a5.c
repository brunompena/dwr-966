/******************************************************************************
**
** FILE NAME    : ppa_datapath_xrx200_a5.c
** PROJECT      : UEIP
...
#include "ppa_datapath_fw_xrx200_a5.h"
#include "ppa_hal_xrx200_a5.h"
#include "ppa_xrx200_a5_common.h"
...
inline int ltq_mei_atm_showtime_check_local(const unsigned char line_idx, int *is_showtime, struct port_cell_info *port_cell, void **xdata_addr) 
{ 
    if ( is_showtime != NULL ) 
        *is_showtime = 1; 
...
static INLINE void reset_ppe(void)
{
    ...
    ltq_rcu_w32_mask(0, BIT(3), RCU_RST_REQ);
    ltq_rcu_w32_mask(0, BIT(11), RCU_RST_REQ);
    udelay(1000);
...
static INLINE int pp32_start(int pp32)
{
    ...
    /*  download firmware   */
    ret = pp32_download_code(pp32, vr9_a5_fw_code, sizeof(vr9_a5_fw_code) / sizeof(*vr9_a5_fw_code), vr9_a5_fw_data, sizeof(vr9_a5_fw_data) / sizeof(*vr9_a5_fw_data));
    if ( ret != PPA_SUCCESS )
        return ret;
...
static int proc_write_queue_seq(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    return count;
}
...
static int __init ppe_eth_init(void)
{
    ...
    for ( i = 0; i < ATM_PORT_NUMBER; i++ )
    {
        g_atm_priv_data.port[i].dev = atm_dev_register("ifx_atm", NULL, &g_ppe_atm_ops, -1, 0UL);

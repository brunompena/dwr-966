...
/*
    uint32_t flags[] = {
        ...
        0,
        GEN_MODE_CFG1->sys_cfg == 0 ? PPA_PHYS_PORT_FLAGS_MODE_ATM_WAN_VALID : 0
    };
 */
    ...
    if(p_flags)
    {
        *p_flags = 0;
        switch(port)
        {
            ...
            case 7:
                if(*CFG_WAN_PORTMAP & (1 << port)){
                    *p_flags = PPA_PHYS_PORT_FLAGS_MODE_ATM_WAN_VALID;
                }
                break;
...
//    cfg = *BRG_TBL_CFG;
//
//    if ( (flags & PPA_SET_BRIDGING_CFG_ENTRY_NUM) )
//    {
...
//    }
//
////    if ( (flags & PPA_SET_BRIDGING_CFG_BR_TO_SRC_PORT_EN) )
////        cfg.br_to_src_port_en = (cfg.br_to_src_port_en & ~br_to_src_port_mask) | (br_to_src_port_en & br_to_src_port_mask);
//
////    if ( (flags & PPA_SET_BRIDGING_CFG_DEST_VLAN_EN) )
////        cfg.dest_vlan_en = f_dest_vlan_en ? 1 : 0;
//
////    if ( (flags & PPA_SET_BRIDGING_CFG_SRC_VLAN_EN) )
////        cfg.src_vlan_en = f_src_vlan_en ? 1 : 0;
//
////    if ( (flags & PPA_SET_BRIDGING_CFG_MAC_CHANGE_DROP) )
                          ...
                          uint8_t *collion_flag)
{
    ...
    struct rout_forward_compare_tbl compare = {0};
    ...
    else
    {
        ...
        if ( (*CFG_WAN_PORTMAP & PPA_DEST_LIST_ATM) )   /*DSL WAN mode */
...
#if 1
    //why PPE FW need to map port ID 6/7 to CPU port ( for A5 only ), for PVC reason ?
...
//    mask[3]     |= 0x07 << 5;
    ...
    if ( (*CFG_WAN_PORTMAP & PPA_DEST_LIST_ATM) /*DSL WAN mode */ && (p_vlan->rx_if_id == 6 || p_vlan->rx_if_id == 7) )
    {

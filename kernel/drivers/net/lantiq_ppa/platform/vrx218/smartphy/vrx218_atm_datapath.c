...
static int ppe_open(struct atm_vcc *vcc)
{
    ...
    for ( sw_tx_queue = 0; sw_tx_queue < ATM_SW_TX_QUEUE_NUMBER; sw_tx_queue++ )
        if ( !(g_atm_priv_data.sw_tx_queue_table & (1 << sw_tx_queue)) )
            break;
    if ( sw_tx_queue == ATM_SW_TX_QUEUE_NUMBER )
    {
    ...
    /*  allocate PVC    */
    for ( conn = 0; conn < ATM_PVC_NUMBER; conn++ )
...
static int atm_push(struct sk_buff *skb, struct flag_header *header, unsigned int ifid)
{
    if ( ifid == 6 /* IPoA/PPPoA */ || ifid == 7 /* EoA */ )
    {
        ...
        if ( conn < 0 || conn >= ATM_PVC_NUMBER || (vcc = g_atm_priv_data.connection[conn].vcc) != NULL )
        {
...
static int sw_tx_queue_add(int conn)
{
    ...
    for ( new_sw_tx_queue = 0; new_sw_tx_queue < ATM_SW_TX_QUEUE_NUMBER; new_sw_tx_queue++ )
        if ( !(g_atm_priv_data.sw_tx_queue_table & (1 << new_sw_tx_queue)) )
            break;
    if ( new_sw_tx_queue == ATM_SW_TX_QUEUE_NUMBER )
    {
        ret = -EINVAL;
...
static int sw_tx_queue_del(int conn)
{
    ...
    for ( sw_tx_queue_table = g_atm_priv_data.connection[conn].sw_tx_queue_table, rem_sw_tx_queue = ATM_SW_TX_QUEUE_NUMBER - 1;
          (sw_tx_queue_table & (1 << (ATM_SW_TX_QUEUE_NUMBER - 1))) == 0;
    ...
    for ( sw_tx_queue_table = new_sw_tx_queue_table, sw_tx_queue = ATM_SW_TX_QUEUE_NUMBER - 1;
          (sw_tx_queue_table & (1 << (ATM_SW_TX_QUEUE_NUMBER - 1))) == 0;
          sw_tx_queue_table <<= 1, sw_tx_queue-- );

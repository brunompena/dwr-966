...
//    outq_qos_cfg_ctxt_t outq_qos_cfg_ctxt;
    ...
    qosq_cfg_ctxt.des_num   = 32;

    for( i = 0; i < 16; i ++) {
        qosq_cfg_ctxt.des_base_addr = __US_QOSQ_DES_LIST_BASE + (i * 64);
...
void vrx218_atm_local_des_init(unsigned long base)
{
    ...
    tx_descriptor_t tx_descriptor;
    rx_descriptor_t rx_descriptor;
    unsigned int all5_sb_addr[13] = { 0x2100, 0x2C00, 0x3000, 0x3190, 0x3D80, 0x44E0, 0x4670, 0x4800, 0x4990, 0x4D40, 0x5B00, 0x5C90, 0x5E20 };
    ...
    for (i=0; i<16; i++) {
        tx_descriptor.data_ptr = ((PDBRAM_TX_PKT_BUFFER_BASE + (i * 1600))/4);
    ...
    for (i=0; i<13; i++) {
        rx_descriptor.data_ptr = (PDBRAM_TX_PKT_BUFFER_BASE + (16 * 1600) + (i * 1600));
        dst_addr = (void *)SOC_ACCESS_VRX218_SB((__DS_TC_LOCAL_AAL5Q_DES_LIST_BASE + (i * 2)), base);
        dword_mem_write(dst_addr, &rx_descriptor, sizeof(rx_descriptor));
    }
    for (i=0; i<13; i++) {
        rx_descriptor.data_ptr = (VRX_SB_MEMORY(all5_sb_addr[i] - 0x2000));
        dst_addr = (void *)SOC_ACCESS_VRX218_SB((__DS_TC_LOCAL_AAL5Q_DES_LIST_BASE + (13 * 2) + (i * 2)), base);
        dword_mem_write(dst_addr, &rx_descriptor, sizeof(rx_descriptor));
    }
    ...
    for (i=0; i<10; i++) {
        ...
        rx_descriptor.data_ptr = (VRX_SB_MEMORY(oam_sb_addr[i] - 0x2000));
...
void vrx218_atm_edma_copy_ch_init(unsigned long base)
{
    ...
    for (i=0; i<16; i++) {
#if 0
        local_desq_cfg_ctxt.des_num = (i == 0)?16:0;
#else
        local_desq_cfg_ctxt.des_num = 1;
#endif
    ...
    local_desq_cfg_ctxt.des_num = 26;

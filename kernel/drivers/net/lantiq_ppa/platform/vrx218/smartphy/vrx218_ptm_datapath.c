...
#ifdef CONFIG_IFX_LED
    g_wrx_bc_user_cw = *RECEIVE_NON_IDLE_CELL_CNT(0, g_vrx218_dev_us.phy_membase) + *RECEIVE_NON_IDLE_CELL_CNT(1, g_vrx218_dev_us.phy_membase);
    g_total_tx_cw = *TRANSMIT_CELL_CNT(0, g_vrx218_dev_us.phy_membase) + *TRANSMIT_CELL_CNT(1, g_vrx218_dev_us.phy_membase);
    if ( g_dsl_bonding ) {
...
static int ptm_qos_hard_start_xmit_b(struct sk_buff *skb, struct net_device *dev)
{
    ...
    swap_mac(skb->data);
    eth_xmit(skb, PTM_PORT /* DSL */, 2, 2, get_class_by_qid(PTM_PORT, qid),0);
    return 0;
}

...
#else
//    extern u32 clk_get_rate (struct clk*);
#endif
    ...
    static uint32_t set_qos_port_id(void)
    {
        ...
        if( wan_itf & PPA_DEST_LIST_ETH0 ) qos_queue_portid = 0;
        else if( wan_itf & PPA_DEST_LIST_ETH1 ) qos_queue_portid = 1;
        //else if ( E5 ptm ) qos_queue_portid = 7;
        else
        {
            dbg("Configured wan_itf does not support QoS:%x\n", wan_itf);
...
#ifdef CONFIG_LTQ_PPA_QOS_RPOC_PROC
int proc_read_qos(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    struct clk * clk; 
    clk = clk_get_ppe(); 
    llen = sprintf(pstr, "  PPE clk=%u MHz, basic tick=%u\n",(u32)clk_get_rate(clk)/1000000, (u32)get_basic_time_tick());
#endif
...
/*
    uint32_t flags[] = {
        GEN_MODE_CFG1->sys_cfg == 2 ? PPA_PHYS_PORT_FLAGS_MODE_ETH_MIX_VALID : PPA_PHYS_PORT_FLAGS_MODE_ETH_LAN_VALID,
        GEN_MODE_CFG1->sys_cfg == 3 ? PPA_PHYS_PORT_FLAGS_MODE_ETH_WAN_VALID : 0,
...
{ /*hash_table_id: note, in GRX500, LAN/WAN will share same hash table, so we add this variable for it */
    uint32_t hash;
...
#ifdef TMP_DBG
    printk("  get_hash f_is_lan              = %d\n", hash_info->f_is_lan);
    printk("  get_hash src_ip                = %d.%d.%d.%d\n", hash_info->src_ip >> 24, (hash_info->src_ip >> 16) & 0xFF, (hash_info->src_ip >> 8) & 0xFF, hash_info->src_ip & 0xFF);
    printk("  get_hash src_port              = %d\n", hash_info->src_port);
    printk("  get_hash dst_ip                = %d.%d.%d.%d\n", hash_info->dst_ip >> 24, (hash_info->dst_ip >> 16) & 0xFF, (hash_info->dst_ip >> 8) & 0xFF, hash_info->dst_ip & 0xFF);
    printk("  get_hash dst_port              = %d\n", hash_info->dst_port);
    printk("  get_hash hash_index            = %d\n", hash_info->hash_index);
    printk("  get_hash hash_table_id         = %d\n", hash_info->hash_table_id);
#endif
    return PPA_SUCCESS;
...
uint32_t add_dslite_tunnel_entry(struct net_device *dev, uint32_t *tunnel_idx)
{
    ...
    if(entry >= MAX_DSLITE_TUNNEL_ENTRIES){
        ...
        ip6_hdr.saddr = t->parms.laddr;
        ip6_hdr.daddr = t->parms.raddr;
        *p_ip6hdr = ip6_hdr;
    }
...
int32_t ppe_drv_del_multifield_entry_via_index(int32_t index, uint32_t flag);
...
*/
uint8_t ppe_drv_multifield_control(uint8_t enable, uint32_t flag)
{
...
/*!
    ...
    \return uint8_t, The return value can be any one of the following:  \n
               ...
               PPA_FAILURE on error
*/
uint8_t ppe_drv_get_multifield_status(uint32_t *enable, uint32_t flag)
{
...
*/
int32_t ppe_drv_get_multifield_max_entry(uint32_t flag)
{
...
*/
int32_t ppe_drv_add_multifield_entry(PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, int32_t *index, uint32_t flag)
{
    int ret=PPA_FAILURE;
    int32_t entry, empty_entry = -1;
    ...
    {
        dbg("ppe_drv_add_multifield_entry mfe_param2word fail\n");
        return PPA_EAGAIN;
    }
...
ADD_MFE_ENTRY_GOON:
    ...
    *index = entry;
    dbg("ppe_drv_add_multifield_entry ok with index=%x\n", *index);
    return PPA_SUCCESS;
...
/*!
    ...
    \return int32_t, return the bytes in the compare table. It can be any one of the following now: \n
               ...
               PPA_FAILURE on error
*/
int32_t ppe_drv_get_multifield_entry(int32_t index, PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag)
{
    int ret=PPA_SUCCESS;
    uint32_t compare[4] = {0};
    uint32_t mask[4] = {0};
...
*/
int32_t ppe_drv_del_multifield_entry(PPA_MULTIFIELD_FLOW_INFO *p_multifield_info, uint32_t flag)
{
    int ret=PPA_FAILURE;
    int32_t entry;
    uint32_t compare[4] = {0};
    ...
    if( mfe_param2word(p_multifield_info, compare, mask, &act) != PPA_SUCCESS )
    {
        dbg("ppe_drv_del_multifield_entry mfe_param2word fail\n");
        return PPA_EAGAIN;
    }
    ...
    for ( entry = MAX_CLASSIFICATION_ENTRIES - 1; entry >= 0; entry-- )
    {
        if (g_classification_entry_counter[entry] )
        {
           if ( ppa_memcmp( (void *)CLASSIFICATION_CMP_TBL(entry), (void *)compare, sizeof(compare)) != 0 ||
            ...
            dbg("ppe_drv_del_multifield_entry found matched one with mfe[%d]. So delete it\n", entry );

            ppe_lock_release(&g_classification_lock);
            if ( p_multifield_info->cfg0.vlan_info.vlan_info_manual.action_out_vlan_insert )
                del_outer_vlan_entry(act.out_vlan_ix);

            ppe_drv_del_multifield_entry_via_index(entry, flag );

            return  PPA_SUCCESS;
        }
    }
   dbg("ppe_drv_del_multifield_entry failed to find a matched one\n" );
   //not found
   ppe_lock_release(&g_classification_lock);
   if ( p_multifield_info->cfg0.vlan_info.vlan_info_manual.action_out_vlan_insert )
        del_outer_vlan_entry(act.out_vlan_ix);
...
static int32_t get_basic_time_tick(void)
{
    ...
    struct clk *clk;
    clk = clk_get_ppe(); 
    return TX_QOS_CFG->time_tick / (clk_get_rate(clk) /1000000);
#endif
}
...
int32_t get_qos_qnum( uint32_t portid, uint32_t flag)
{
    if( set_qos_port_id() != PPA_SUCCESS ) 
    {
        dbg("set_qos_port_id fail\n");
        return 0;
    }
    if ( qos_queue_portid != portid )
    {
        dbg("qos_queue_portid(%d) != portid(%d)\n", qos_queue_portid, portid  );
        return 0;
    }
...
int32_t get_qos_status( PPA_QOS_STATUS *stat)
{
    ...
    struct clk *clk;
    clk = clk_get_ppe(); 
    stat->pp32_clk = clk_get_rate(clk);
#endif
   ...
   case PPA_GENERIC_HAL_GET_MAX_ENTRIES:
        {
            ...
            entry->max_mf_flow = MAX_CLASSIFICATION_ENTRIES;
...
//Fix warning message when exports API from different PPE FW Driver--begin
   ...
   case PPA_GENERIC_HAL_GET_CAPWAP_MIB:
    ...
    case PPA_GENERIC_HAL_MFE_ADD_FLOW:   //add a multiple field vlan editing entry
        {
            ...
            return ppe_drv_add_multifield_entry( &flow->multifield_info, &flow->entry, flow->flag);
        }

    case PPA_GENERIC_HAL_MFE_DEL_FLOW:   //del a multiple field vlan editing entry
        {
            PPE_MULTIFILED_FLOW *flow=(PPE_MULTIFILED_FLOW *)buffer;
            return ppe_drv_del_multifield_entry( &flow->multifield_info, flow->flag);
        }

    case PPA_GENERIC_HAL_MFE_DEL_FLOW_VIA_ENTRY:  //del a multiple field vlan editing entry
        {
            PPE_MULTIFILED_FLOW *flow=(PPE_MULTIFILED_FLOW *)buffer;
            return ppe_drv_del_multifield_entry_via_index( flow->entry, flow->flag);
        }

    case PPA_GENERIC_HAL_MFE_GET_FLOW:  //get a multiple field vlan editing entr
        {
            PPE_MULTIFILED_FLOW *flow=(PPE_MULTIFILED_FLOW *)buffer;
            return ppe_drv_get_multifield_entry( flow->entry, &flow->multifield_info, flow->flag);
        }
    ...
    case PPA_GENERIC_HAL_SET_WANITF: // init QOS Rateshapping
        {
...
static inline void hal_init(void)
{
    ...
    *KEY_SEL_n(3) = 0x52430000;
    GEN_MODE_CFG1->classification_num = MAX_CLASSIFICATION_ENTRIES;
    GEN_MODE_CFG1->brg_class_en          = 0; // 1;

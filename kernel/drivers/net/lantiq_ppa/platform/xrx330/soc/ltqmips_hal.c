...
static void add_tunnel_6rd_header(struct net_device *dev, volatile struct iphdr *iph);
  ...
  extern u32 ppa_get_pp32_clock(void);
#endif
  ...
  #define PPE_MAX_ETH1_QOS_QUEUE 8
        ...
        #define PPA_DRV_QOS_WFQ_WLEVEL_2_W  ( 2000 )
...
#else
    clk = clk_get_ppe();
    llen = sprintf(pstr, "  PPE clk=%u MHz, basic tick=%u\n", (unsigned int)(clk_get_rate(clk) / 1000000), (unsigned int) get_basic_time_tick() );
#endif
    pstr += llen;
    len += llen;
#ifdef CONFIG_LTQ_PPA_QOS_WFQ
...
/*
  * Set phy wan port mode (ATM or PTM)
  * Get phy wan port flag by port
  * xmode == 0  Get
  * xmode == 1  Set
  * wan_mode : -1 //not setting from VRX318,  0- ATM wan mode,  1- PTM wan mode
 */
int xet_phy_wan_port(uint32_t port, uint32_t *flags, uint32_t wanmode, int xmode)
{
    static int wan_mode = -1;
    
    if(xmode == 0){//Get
        if(wan_mode == -1){
            *flags = 0;
        }else if(wan_mode == 0){ //ATM
            *flags = PPA_PHYS_PORT_FLAGS_MODE_ATM_WAN_VALID; 
        }else if(wan_mode == 1){
            *flags = PPA_PHYS_PORT_FLAGS_MODE_ETH_WAN_VALID;
        }
    }else if(xmode == 1){//Set
        if(wanmode == 0 || wanmode == 1){
            wan_mode = wanmode;
        }
    }
...
EXPORT_SYMBOL(xet_phy_wan_port);
   ...
   if(p_flags)
    {
        *p_flags = 0;
        switch(port)
        {
            ...
            case 4:
            case 5:
            case 6: //port 6 is always reserved for DSL WAN mode
                if(port == 6 && *CFG_WAN_PORTMAP & (1 << 7)){//DSL wan
                    break;
                }
            ...
            case 7: 
			    if(*CFG_WAN_PORTMAP & (1 << port)){
				    //*p_flags = PPA_PHYS_PORT_FLAGS_MODE_ETH_WAN_VALID;
				    xet_phy_wan_port(port,p_flags,0,0);
                    if(*p_flags == PPA_PHYS_PORT_FLAGS_MODE_ETH_WAN_VALID && ifname){
                        strcpy(ifname, "ptm0");
                    }
				}else{
                    *p_flags = PPA_PHYS_PORT_FLAGS_MODE_EXT_LAN_VALID | PPA_PHYS_PORT_FLAGS_EXT_CPU0;
                          ...
                          uint8_t *collion_flag)
{
    ...
    if(ppa_drv_hal_get_mpoa_type_hook != NULL){//ATM force to remove outer vlan which is used for PVCs differentiation for normal path
        f_out_vlan_rm_enable = 1;
    }
    ...
    if(f_is_lan){
        if(ppa_drv_hal_get_mpoa_type_hook && !ppa_drv_hal_get_mpoa_type_hook(dslwan_qid, &mpoa_type)){
            action.mpoa_type  = mpoa_type;
        }
    }
...
//    unsigned int    acc_en              :1; 
    ...
    //CAPWAP_CONFIG_TBL(entry)->da_mac_lo =( (*(dst_mac + 4) << 24) | (*(dst_mac + 5) << 16));
...
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 10, 7)
    return TX_QOS_CFG->time_tick / (ppa_get_pp32_clock() /1000000);
#else
    struct clk * clk ;
    clk = clk_get_ppe();
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 10, 7)
    stat->pp32_clk = ppa_get_pp32_clock();
#else
    clk = clk_get_ppe();
...
static int32_t ppa_hal_generic_hook(PPA_GENERIC_HOOK_CMD cmd, void *buffer, uint32_t flag)
{
    ...
    if ( PS_MC_GENCFG3->vrx218_en ) {
        ...
        extern ppe_generic_hook_t ifx_ppe_ext_hal_generic_hook;
        ...
        case PPA_GENERIC_HAL_GET_PPE_FW_VERSION:
            {
                ...
                if ( version->index == 0 )
                    break;
                version->index -= 1;
                ret = ifx_ppe_ext_datapath_generic_hook != NULL ? ifx_ppe_ext_datapath_generic_hook(PPA_GENERIC_DATAPATH_GET_PPE_VERION, buffer, flag) : PPA_FAILURE;
                version->index += 1;
                return ret;
            }
        ...
        case PPA_GENERIC_HAL_CLEAR_DSL_MIB:
            if ( PS_MC_GENCFG3->vrx218_atm )
                return ifx_ppe_ext_hal_generic_hook != NULL ? ifx_ppe_ext_hal_generic_hook(cmd, buffer, flag) : PPA_FAILURE;
            else
                return PPA_FAILURE;
        ...
        case PPA_GENERIC_HAL_GET_QOS_STATUS:    //get QOS status
            {
                //  fill SOC information before passing to VRX318
                PPA_QOS_STATUS *stat = (PPA_QOS_STATUS *)buffer;
                stat->wan_port_map = *CFG_WAN_PORTMAP;
                stat->wan_mix_map = *CFG_MIXED_PORTMAP;
            }
 ...
 #endif
            if ( *(uint32_t *)buffer != PPA_WAN_QOS_PTM0 )  //check whether port ID is DSL WAN port
                break;
 #ifdef CONFIG_LTQ_PPA_QOS_WFQ
        case PPA_GENERIC_HAL_INIT_QOS_WFQ:      // init WFQ
 #endif
 #ifdef CONFIG_LTQ_PPA_QOS_RATE_SHAPING
   ...
   case PPA_GENERIC_HAL_DEL_CAPWAP_ENTRY:
        {
            ...
            //struct capwap_group_list_item *capwap = (struct capwap_group_list_item *)buffer;

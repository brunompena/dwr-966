...
static INLINE int32_t ppa_pkt_filter(PPA_BUF *ppa_buf, uint32_t flags)
{
    ...
    if ( ppa_get_pkt_ip_proto(ppa_buf) != PPA_IPPROTO_UDP && ppa_get_pkt_ip_proto(ppa_buf) != PPA_IPPROTO_TCP )
    {
...
int32_t ppa_session_add(PPA_BUF *ppa_buf, PPA_SESSION *p_session, uint32_t flags)
{
    ...
    if((ret = ppa_speed_handle_frame(ppa_buf, p_session, flags)) == PPA_SESSION_FILTED){
        return ret;
    }
    ...
    if ( (flags & PPA_F_BEFORE_NAT_TRANSFORM) )
    {
        if ( ppa_add_session(ppa_buf, p_session, flags) != PPA_SUCCESS )
        {
    ...
    else
    {
        ...
        PPA_SYNC();
        ret = ppa_update_session(ppa_buf, p_session, flags);
        return ret;
...
#ifdef CONFIG_LTQ_PPA_API_SW_FASTPATH
		    // session was in hardware an the modification failed; so the session is moved out of HW and put in SW fastpath 
                    p_item->flags |= SESSION_ADDED_IN_SW;
#endif
                    goto __MODIFY_DONE;
                }
...
int32_t ppa_inactivity_status(PPA_U_SESSION *p_session)
{
    ...
    if ( (ret == PPA_SESSION_EXISTS && p_item->ip_proto == PPA_IPPROTO_TCP) 
    ...
    if ( ret == PPA_SESSION_EXISTS && (p_item->flags & (SESSION_ADDED_IN_HW | SESSION_ADDED_IN_SW)) )
    {
        uint32_t tmp;
    ...
    if ( ret_reply == PPA_SESSION_EXISTS && (p_item_reply->flags & (SESSION_ADDED_IN_HW | SESSION_ADDED_IN_SW)) )
    {
        uint32_t tmp;
...
int32_t ppa_set_session_inactivity(PPA_U_SESSION *p_session, int32_t timeout)
{
    ...
    if (ret_reply == PPA_SESSION_EXISTS){
...
int32_t ppa_bridge_entry_add(uint8_t *mac_addr, PPA_NETIF *netif, uint32_t flags)
{
    struct bridging_session_list_item *p_item;
    int32_t ret = PPA_SESSION_ADDED;
    

    if( !g_bridging_mac_learning ) return PPA_FAILURE;

    ppa_br_get_htable_lock();
    if ( __ppa_bridging_lookup_session(mac_addr, netif, &p_item) == PPA_SESSION_EXISTS )
    {
...
int32_t ppa_bridge_entry_delete(uint8_t *mac_addr, uint32_t flags)
{
    struct bridging_session_list_item *p_item;
    int32_t ret = PPA_FAILURE;
    ...
    ppa_br_get_htable_lock();
    if ( __ppa_bridging_lookup_session(mac_addr, NULL, &p_item) != PPA_SESSION_EXISTS ){
...
int32_t ppa_bridge_entry_hit_time(uint8_t *mac_addr, uint32_t *p_hit_time)
{
    struct bridging_session_list_item *p_item;
...
int32_t ppa_bridge_entry_inactivity_status(uint8_t *mac_addr)
{
    struct bridging_session_list_item *p_item;
    int32_t ret = PPA_HIT;
...
int32_t ppa_set_bridge_entry_timeout(uint8_t *mac_addr, uint32_t timeout)
{
    struct bridging_session_list_item *p_item;

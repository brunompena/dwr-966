...
#else
static int ppa_dev_ioctl(struct file *, unsigned int, unsigned long);
#endif
    ...
    case PPA_CMD_ADD_MAC_ENTRY:
        {
            ...
            if ( ppa_bridge_entry_add(cmd_info->mac_entry.mac_addr, netif, cmd_info->mac_entry.flags) != PPA_SESSION_ADDED )
                goto EXIT_EIO;
            else
                goto EXIT_ZERO;
        }
    ...
    case PPA_CMD_DEL_MAC_ENTRY:
        {
            ...
            if ( ppa_bridge_entry_delete(cmd_info->mac_entry.mac_addr, cmd_info->mac_entry.flags) != PPA_SUCCESS )
                goto EXIT_EIO;
            else
                goto EXIT_ZERO;
        }
            ...
            if ( ppa_netif_start_iteration(&pos, &ifinfo) == PPA_SUCCESS )
            {
                do
                {
                    if ( (ifinfo->flags & flag_mask) )
                    {
                        ...
                        cmd_info->all_if_info.ifinfo[i].acc_rx = ifinfo->acc_rx - ifinfo->prev_clear_acc_rx;
                        cmd_info->all_if_info.ifinfo[i].acc_tx = ifinfo->acc_tx - ifinfo->prev_clear_acc_tx;
#endif
    ...
    case PPA_CMD_GET_PPA_HASH_SUMMARY:
        {
            ...
            {
                cmd_info->ppa_hash_info.hash_info.count[0] = get_ppa_sesssion_num_in_one_hash(i);
    ...
    case PPA_CMD_GET_MAXCOUNT_CAPWAP:
#endif
        {
            ...
            if( cmd == PPA_CMD_GET_COUNT_LAN_SESSION )
                cmd_info->count_info.count = ppa_get_routing_session_count( 1,  cmd_info->count_info.flag, cmd_info->count_info.hash_index) ;
            else if( cmd == PPA_CMD_GET_COUNT_WAN_SESSION )
                cmd_info->count_info.count = ppa_get_routing_session_count(0,  cmd_info->count_info.flag, cmd_info->count_info.hash_index ) ;
            else if( cmd == PPA_CMD_GET_COUNT_NONE_LAN_WAN_SESSION )
                cmd_info->count_info.count = ppa_get_routing_session_count(2,  cmd_info->count_info.flag, cmd_info->count_info.hash_index ) ;                
             else if( cmd == PPA_CMD_GET_COUNT_LAN_WAN_SESSION )
                cmd_info->count_info.count = ppa_get_routing_session_count( 1,  cmd_info->count_info.flag, cmd_info->count_info.hash_index) + ppa_get_routing_session_count(0,  cmd_info->count_info.flag, cmd_info->count_info.hash_index );
    ...
    case PPA_CMD_GET_LAN_WAN_SESSIONS:
        {
           ...
           if( cmd_info->session_info.count_info.hash_index )
           {
                pos = cmd_info->session_info.count_info.hash_index -1;
           }
           if( cmd_info->session_info.count_info.count == 0 ) goto EXIT_EIO;            
           do
           {    
           #if defined(FIX_MEM_SIZE)
              ...
              buff_len = NUM_ENTITY(tmp_session_buf);
                ...
                if( (ret == ppa_session_get_items_via_index(pos,
                                                &p_item_one_index,buff_len,
                                                &tmp_num,
                                                cmd_info->session_info.count_info.stamp_flag)) == PPA_INDEX_OVERFLOW ) //end of list
                {   
                    ret = PPA_SUCCESS;
                    break;
                }
...
( (cmd_info->session_info.count_info.flag == SESSION_ADDED_IN_SW) && (pp_item->flags & SESSION_ADDED_IN_SW) ) ||
                            ( (cmd_info->session_info.count_info.flag == SESSION_NON_ACCE_MASK) && !(pp_item->flags & SESSION_ADDED_IN_HW ) && !(pp_item->flags & SESSION_ADDED_IN_SW ) ) )/*get all software-acelerated sessions*/
                        {
                            ...
                            ppa_memcpy( &cmd_info->session_info.session_list[0].src_ip, &pp_item->src_ip, sizeof(cmd_info->session_info.session_list[0].src_ip) );
                            cmd_info->session_info.session_list[0].src_port = pp_item->src_port;
                            ppa_memcpy( &cmd_info->session_info.session_list[0].dst_ip, &pp_item->dst_ip, sizeof(cmd_info->session_info.session_list[0].dst_ip) );
                            cmd_info->session_info.session_list[0].dst_port = pp_item->dst_port;
                            ppa_memcpy( &cmd_info->session_info.session_list[0].nat_ip, &pp_item->nat_ip, sizeof(cmd_info->session_info.session_list[0].nat_ip) );;
                            ...
                            cmd_info->session_info.session_list[0].flag2 = pp_item->flag2;
                            cmd_info->session_info.session_list[0].mips_bytes = pp_item->mips_bytes - pp_item->prev_clear_mips_bytes;
                            cmd_info->session_info.session_list[0].hw_bytes = pp_item->acc_bytes - pp_item->prev_clear_acc_bytes;
                            cmd_info->session_info.session_list[0].session = (uint32_t ) pp_item->session;
                            cmd_info->session_info.session_list[0].session = (uint32_t ) pp_item->session;
                            cmd_info->session_info.session_list[0].collision_flag = (uint32_t ) pp_item->collision_flag;
                            cmd_info->session_info.session_list[0].priority = (uint32_t ) pp_item->priority;
#if defined(CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS)
			    if ( ppa_get_session_limit_enable(0) )
	                            cmd_info->session_info.session_list[0].session_priority = (uint32_t ) pp_item->session_priority;
			    else
	                            cmd_info->session_info.session_list[0].session_priority = 0;
                            ...
                            cmd_info->session_info.session_list[0].hash_index = (uint32_t ) pp_item->hash_index;
                            cmd_info->session_info.session_list[0].hash_table_id = (uint32_t ) pp_item->hash_table_id;                            
                        #endif     
                            cmd_info->session_info.session_list[0].prev_sess_bytes = pp_item->prev_sess_bytes;
                ...
                if( cmd_info->session_info.count_info.hash_index ) break;
           } while(count < cmd_info->session_info.count_info.count );
#if defined(FIX_MEM_SIZE)           
#else
           if( p_item_one_index )
            {
...
int32_t ppa_ioctl_add_session(unsigned int cmd, unsigned long arg, PPA_CMD_DATA *cmd_info)
{
    ...
    if ( ppa_session_start_iteration(&pos, &p_item) == PPA_SUCCESS )  {
        do {
        ...
        } while ( ppa_session_iterate_next(&pos, &p_item) == PPA_SUCCESS  );
    }
    else {
    ...
    if( cmd_info->detail_session_info.flags & SESSION_IS_IPV6 ) {
        ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT, "add session:%s from %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%5d to %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%5d\n",
            (cmd_info->detail_session_info.ip_proto == PPA_IPPROTO_TCP)?"tcp":"udp",
    ...
    __ppa_insert_session_item( p_item ); //don't
...
{
    struct session_list_item *p_item;
    uint32_t pos = 0, n_match=0, f_lan_wan_match;

#if defined(CONFIG_LTQ_PPA_API_SW_FASTPATH)
    uint32_t old_flag=0, ret=0;
#endif
    ...
    if( cmd_info->session_extra_info.flags == 0 ) return PPA_SUCCESS; //no information need to update

    if ( ppa_session_start_iteration(&pos, &p_item) == PPA_SUCCESS )
    {
        do
        {
            if( ( cmd_info->session_extra_info.session !=0 ) && ( cmd_info->session_extra_info.session != (uint32_t)p_item->session ) ) continue; //not match
            ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"ConnTrack matched %x match with %x\n", cmd_info->session_extra_info.session, (uint32_t)p_item->session );


            f_lan_wan_match = 0;
            if( ( cmd_info->session_extra_info.lan_wan_flags & SESSION_WAN_ENTRY ) )  //WAN flag is set
            {
                if( ! ( p_item->flags & SESSION_WAN_ENTRY ) )
                {
                    ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"Not WAN session\n");
                }
                else
                {
                    f_lan_wan_match = 1;
                    ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"WAN session %x match\n", (uint32_t)p_item->session );
                }
            }

            if( ( cmd_info->session_extra_info.lan_wan_flags & SESSION_LAN_ENTRY ) )  //WAN flag is set
            {
                if( ! ( p_item->flags & SESSION_LAN_ENTRY ) )
                {
                    ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"Not LAN session\n");
                }
                else
                {
                    f_lan_wan_match = 1;
                    ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"LAN session %x match\n", (uint32_t) p_item->session );
                }
            }
            if( ! f_lan_wan_match ) continue;

            n_match ++;
            /*below code copy from ppa_session_modify ---start*/
            ppa_update_session_extra( &cmd_info->session_extra_info.session_extra, p_item, cmd_info->session_extra_info.flags);

            if( cmd_info->session_extra_info.flags & PPA_F_ACCEL_MODE) 
            {  //only need to remove or add to accleration path and no other information need to update
			if ( (p_item->flags & SESSION_ADDED_IN_HW) && ( p_item->flags & SESSION_NOT_ACCEL_FOR_MGM) )
            ...
            else if ( cmd_info->session_extra_info.flags != 0)
            {   //update parameters to PPE FW SESSION TABLE
                if ( ppa_hw_update_session_extra(p_item, cmd_info->session_extra_info.flags) != PPA_SUCCESS )
                {
                    //  update failed
                    ppa_hw_del_session(p_item);
                    p_item->flags &= ~SESSION_ADDED_IN_HW;                        
    ...
    ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"There are %d sessions matched\n", n_match);

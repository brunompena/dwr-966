...
// to ckeck the global hardware acceleration enabled flag
static INLINE int32_t ppa_hw_accelerate(PPA_BUF *ppa_buf, PPA_SESSION *p_session);
...
#else
        ppa_hlist_for_each_entry_safe( p_item, tmp, &g_session_list_hash_table[idx], hlist){
#endif
...
int32_t ppa_session_delete(PPA_SESSION *p_session, uint32_t flags)
{
    ...
    for ( i = 0; i < NUM_ENTITY(flag_template); i++ )
    {
        ...
        if ( __ppa_lookup_session(p_session, i, &p_item) != PPA_SESSION_EXISTS )
            continue;


        dump_list_item(p_item, "ppa_session_delete");
        ppa_remove_session_item(p_item);        
        ppa_free_session_list_item(p_item);
...
int32_t ppa_speed_handle_frame(PPA_BUF *ppa_buf, PPA_SESSION *p_session, uint32_t flags)
{
    struct session_list_item *p_item = NULL;
    int32_t ret = PPA_SESSION_NOT_FILTED;

    ppa_uc_get_htable_lock();
    if ( __ppa_lookup_session(p_session, flags & PPA_F_SESSION_REPLY_DIR, &p_item) == PPA_SESSION_EXISTS ){
...
#if 1
int32_t ppa_update_session(PPA_BUF *ppa_buf, PPA_SESSION *p_session, uint32_t flags)
{
...
#endif
// place holder for some explicit criteria based on which the session must go through SW acceleration
  if(ppa_hw_accelerate(ppa_buf, p_session))
  {
...
int32_t ppa_update_session(PPA_BUF *ppa_buf, PPA_SESSION *p_session, uint32_t flags)
{
    ...
    //  get information needed by hardware/firmware
    if ( (ret = ppa_update_session_info(ppa_buf, &tmp_item, flags)) != PPA_SUCCESS )
    {
        ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"update session fail\n");
        return ret;
    }
...
uint32_t ppa_get_routing_session_count(uint16_t bf_lan, uint32_t count_flag, uint32_t hash_index)
{
    ...
    {          
       ...
       {   
            ...
            count = 0;
            if( (ret == ppa_session_get_items_via_index(pos, &p_item_one_index,0, &tmp_num)) == PPA_INDEX_OVERFLOW ) //end of list
            {   
                break;
            }
            pos ++;

            if( (ret == PPA_ENOMEM) && !p_item_one_index )
            {
...
#else
    ppa_hlist_for_each_entry(p_uc_item,  &g_session_list_hash_table[idx], hlist){
#endif
...
int32_t ppa_add_session(PPA_BUF *ppa_buf, PPA_SESSION *p_session, uint32_t flags)
{
    ...
    if(__ppa_lookup_session(p_session, is_reply, &p_item) == PPA_SESSION_EXISTS){
...
int32_t ppa_update_session_info(PPA_BUF *ppa_buf, struct session_list_item *p_item, uint32_t flags)
{
    ...
    if ( ppa_netif_update(p_item->rx_if, NULL) != PPA_SUCCESS )
    {
    ...
    if ( ppa_netif_update(p_item->tx_if, NULL) != PPA_SUCCESS )
    {
    ...
    if ( ppa_netif_lookup(ppa_get_netif_name(p_item->rx_if), &rx_ifinfo) != PPA_SUCCESS )
    {
   ...
   CLR_DBG_FLAG(p_item, SESSION_DBG_PPE_EDIT_LIMIT);
    ...
    if ( ppa_get_dst_mac(ppa_buf, p_item->session, p_item->dst_mac) != PPA_SUCCESS )
    {
...
#ifdef CONFIG_LTQ_MINI_JUMBO_FRAME_SUPPORT
	tx_mtu =  ppa_get_mtu(p_item->tx_if);
/* Only reduce the session's MTU, if the Tx If cannot handle the exta header bytes for the session - not on global basis */
       if ((g_ppa_ppa_mtu + extra_hdr_bytes) > tx_mtu) {
               p_item->mtu -= (g_ppa_ppa_mtu + extra_hdr_bytes - tx_mtu);
       } 
       else
       {
               p_item->mtu += (tx_mtu - ( g_ppa_ppa_mtu + extra_hdr_bytes ));
       }
 
#endif
PPA_UPDATE_SESSION_DONE_SKIP_MAC:
#if defined(CONFIG_LTQ_PPA_API_SW_FASTPATH)
    swa_ret = ppa_sw_update_session(ppa_buf, p_item, tx_ifinfo);
    if((swa_ret != PPA_SUCCESS ) && (swa_ret != PPA_EINVAL)) {
	ppa_debug(DBG_ENABLE_MASK_DEBUG_PRINT,"session:%x can not get software acceleration parameters!\n",(u32)ppa_get_session(ppa_buf));
...
#else
    ppa_hlist_for_each_entry(p,&g_session_list_hash_table[index],hlist)   
#endif
    {      
        if(i >= count ){
            break;

...
static int _rmnet_map_copy_skb(struct sk_buff *s, struct sk_buff **d)
{
	*d = __netdev_alloc_skb_ip_align(s->dev, s->len + ETH_HLEN, GFP_ATOMIC);
	if (!*d)
		return -ENOMEM;

	skb_reserve(*d, ETH_HLEN);
...
static rx_handler_result_t _rmnet_map_ingress_handler_wnc(struct sk_buff *skb,
					    struct rmnet_phys_ep_conf_s *config)
{
	struct rmnet_logical_ep_conf_s *ep;
	uint8_t mux_id;
					    ...
					    struct rmnet_phys_ep_conf_s *config)
{
	...
	if (config->ingress_data_format & RMNET_INGRESS_FORMAT_DEAGGREGATION) {
		...
		while ((skbn = rmnet_map_deaggregate(skb, config)) != 0) {
			if (rmnet_data_ppa_enable &&
					(RMNET_MAP_GET_MUX_ID(skbn) == 1)) {
				if (!_rmnet_map_copy_skb(skbn, &skbn2))
					_rmnet_map_ingress_handler_wnc(skbn2,
								       config);
			} else {
		...
		if (rmnet_data_ppa_enable && (RMNET_MAP_GET_MUX_ID(skb) == 1))
			rc = _rmnet_map_ingress_handler_wnc(skb, config);
		else

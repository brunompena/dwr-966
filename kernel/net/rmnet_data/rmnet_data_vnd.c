					...
					struct net_device *dev)
{
	...
	if (dev_conf->local_ep.egress_dev) {
		if (rmnet_data_ppa_enable && (dev_conf->local_ep.mux_id == 1))
			 ...
			 const char *prefix)
{
	...
	if (rmnet_data_ppa_enable && (id == 1)) {
		...
		status = ppa_hook_directpath_register_dev_fn(&ppa_ifid, dev,
...
int rmnet_vnd_free_dev(int id)
{
	...
	if (rmnet_data_ppa_enable && (id == 1))
		ppa_hook_directpath_register_dev_fn(&ppa_ifid, dev, NULL, 0);

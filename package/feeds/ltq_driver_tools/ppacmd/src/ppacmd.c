...
static const char ppa_init_short_opts[] = "f:l:w:b:m:c:n:j:k:t:i:h";
#else
static const char ppa_init_short_opts[] = "f:l:w:b:m:c:n:t:i:h";
#endif
#else
#if defined(CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS)
static const char ppa_init_short_opts[] = "f:l:w:b:m:c:n:j:k:t:h";
#else
...
#if defined(CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS)
    {"lan-reserve-collision",    required_argument,  NULL, 'j'},
    {"wan-reserve-collision",    required_argument,  NULL, 'k'},
#endif
...
#if defined(CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS)
            case INIT_CMD_LAN_COLLISION:
                lan_collisions = atoi(val);
                if (lan_collisions < 0)
                {
                    err = ERR_INVALID_COMMAND;
                    goto parse_error;
                }
                pinfo->add_requires_lan_collisions = lan_collisions;
                break;
            
	    case INIT_CMD_WAN_COLLISION:
                wan_collisions = atoi(val);
                if (wan_collisions < 0)
                {
                    err = ERR_INVALID_COMMAND;
                    goto parse_error;
                }
                pinfo->add_requires_wan_collisions = wan_collisions;
                break;
#endif
...
#if defined(CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS)
    pinfo->add_requires_lan_collisions = 0;
    pinfo->add_requires_wan_collisions = 0;
#endif
...
#if defined(CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS)
        case 'j':
            pinfo->add_requires_lan_collisions = str_convert(STRING_TYPE_INTEGER,  popts->optarg, NULL );
            break;

        case 'k':
            pinfo->add_requires_wan_collisions = str_convert(STRING_TYPE_INTEGER,  popts->optarg, NULL );
            break;
#endif
#if defined(MIB_MODE_ENABLE) && MIB_MODE_ENABLE
        case 'i':
...
#if defined(CONFIG_LTQ_PPA_HANDLE_CONNTRACK_SESSIONS)
    IFX_PPACMD_DBG("   LAN Reserve Collisions: %lu\n", pinfo->add_requires_lan_collisions);
    IFX_PPACMD_DBG("   WAN Reserve Collisions: %lu\n", pinfo->add_requires_wan_collisions);
#endif
...
static int ppa_parse_del_session_cmd(PPA_CMD_OPTS *popts, PPA_CMD_DATA *pdata)
{
    ...
    pdata->session_info.session_list[0].ip_proto = PPA_IPPROTO_UDP;
    ...
    while(popts->opt)
    {
        ...
        case 'p':
            ...
            {
                pdata->session_info.session_list[0].ip_proto= PPA_IPPROTO_TCP;
            }
...
#ifndef CONFIG_LTQ_PPA_IPv6_ENABLE
    if( ip1_type == IP_VALID_V6 )
    {
    ...
    else
    {
        ...
        if( pdata->session_info.session_list[0].flags & SESSION_IS_IPV6 )
        {
            IFX_PPACMD_DBG("delete session via ppa session tuple:%s from %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%5d to %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x/%5d\n",
                    (pdata->session_info.session_list[0].ip_proto == PPA_IPPROTO_TCP)?"tcp":"udp",
...
/*ppacmd getwfq ---end*/
#endif  //enod of CONFIG_LTQ_PPA_QOS_WFQ
...
/*ppacmd getrate ---end*/
#endif  //enod of CONFIG_LTQ_PPA_QOS_RATE_SHAPING


#endif /*end if CONFIG_LTQ_PPA_QOS */
...
static int ppa_get_mf_flow_cmd(PPA_COMMAND *pcmd, PPA_CMD_DATA *pdata)
{
    ...
    for(i=start_i; i<end_i; i++)
    {
        ...
        if( ( (res = ppa_do_ioctl_cmd(PPA_CMD_GET_MULTIFIELD, &flow_info ) ) == PPA_CMD_OK ) && ( flow_info.flag == PPA_SUCCESS ) )
        {
            if( !g_output_to_file  )
            {
...
{
    memset( pdata, 0, sizeof(PPA_CMD_PPE_FASTPATH_ENABLE_INFO) );
...
{
    memset( pdata, 0, sizeof(PPA_CMD_SW_FASTPATH_ENABLE_INFO) );
...
static PPA_COMMAND ppa_cmd[] =
{
    ...
    },
    {
        ...
        PPA_CMD_SET_PPE_FASTPATH_ENABLE,
        ppa_set_ppefp_help,
        ppa_set_ppefp_cmd,
        ppa_do_cmd,
        NULL,
    ...
    },    
    {
        ...
        PPA_CMD_GET_PPE_FASTPATH_ENABLE,
        ppa_get_ppefp_help,
        ppa_get_ppefp_cmd,
        ppa_do_cmd,
...
#endif //endof CONFIG_LTQ_PPA_QOS_WFQ
...
#endif //endof CONFIG_LTQ_PPA_QOS_RATE_SHAPING
#endif //end if CONFIG_LTQ_PPA_QOS

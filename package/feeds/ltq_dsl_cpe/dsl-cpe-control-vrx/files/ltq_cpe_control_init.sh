...
start() {
   ...
   if [ "$wan_mode" = "AUTO" -o "$wan_mode" = "ADSL" -o "$wan_mode" = "VDSL" ]; then
	 ...
	 sleep 1
         ...
         # Apply configurations for LineFeatureConfigSet (lfcs) if required,
         # means if one parameter differs from its default value.
         # Defaults configuration values are as follows
         # BS_ENA=1, RETX_ENA=1, VN_ENA=1
         if [ "$BS_ENA" != "1" -o "$RETX_ENA" != "1" -o "$VN_ENA" != "1" ]; then
            dir="0 1"
            for j in $dir ; do
               # Temporary to take care that RETX for upstream direction is disabled
               # by default (also see DSLCPE_SW-768)
               if [ "$j" = "0" ]; then
                  RETX_ENA_CLI=0
               else
                  RETX_ENA_CLI=$RETX_ENA
               fi

...
uname -r | grep -q -e 2.6. -e 3.8. -e 3.10.
if [ $? -eq 0 ]; then
    MODEXT=.ko
fi

drv_dev_base_name=ifx_mei
drv_obj_file_name=drv_mei_cpe_danube$MODEXT
...
if [ "$debug_level" -le 2 ]; then
   echo - create device nodes for ADSL MEI CPE device driver -
fi

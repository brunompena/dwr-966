...
int do_change_boot_partition(void)
{
	...
	char kernel[] = "kernel";
	char rootfs[] = "rootfs";
	char kernelb[] = "kernelb";
	char rootfsb[] = "rootfsb";
	...
	if ((input_value & (0x1 << UBOOT_RESET_GPIO))) //user should press reset button before power on!
		return 1; // don't want to change boot partition.

	// want to change boot partition, should press ant switch button in 10 seconds.		
	 // GPIO 11  LTE_ANT switch
	...
	{
		buf = getenv ("kernelname");
		printf("Switch bootup partition success. \n");
		
		if ( 6 == strlen(buf) )  // kernel
		{
				printf("Switch to kernelb \n");
				setenv("kernelname",  kernelb);
				setenv("rootfsname",  rootfsb);
				udelay(100000);
				saveenv();
				//udelay(1000000);
		}
		else if ( 7 == strlen(buf) )  // kernelb
		{
				printf("Switch to kernel\n");
				setenv("kernelname",  kernel);
				setenv("rootfsname",  rootfs);
				//udelay(100000);
				//saveenv();
				//udelay(1000000);
		}
		...
		{
		printf("Keep origin bootup partition.\n");
		}
		return 0;
}
...
# endif	/* CONFIG_BOOT_RETRY_TIME */
	...
	debug ("### main_loop: bootcmd=\"%s\"\n", s ? s : "<UNDEFINED>");
		...
		buf = getenv ("part1_err");
		part1_err = simple_strtoul(buf, NULL, 16);
		
		buf = getenv ("part2_err");
		part2_err = simple_strtoul(buf, NULL, 16);
	
		printf("part1_err=%d,part2_err=%d\n",part1_err,part2_err);

		if( part1_err == 3 && part2_err == 3)
		{
			printf("Both partition firmware are CRC error, stop booting.. \n");
			run_command("run red_led",0); 
		
		}		
		else
			do_boot_led(NULL, 0, 0, NULL);
		
	if (bootdelay >= 0 && s && !abortboot (bootdelay) && ( part2_err < 3 || part1_err < 3) ) {
# ifdef CONFIG_AUTOBOOT_KEYED
		int prev = disable_ctrlc(1);	/* disable Control C checking */
# endif
		...
		if( partnum == 0 || buf == NULL )  // write default activePartition value
		{
			setenv("kernelname",  kernel);
			setenv("rootfsname",  rootfs);
			sprintf(partition,"1");
		...
		if( do_mem_mc(cmdtp,0,-1,partition) )
		{
			printf("CRC error , changing another booting partition from %s\n",partition);

...
stop_mtlk_drvhlpr()
{
	...
	do
		if [ $count -gt 10 ]
		then
			print2log ERROR "mtlk_drvhlpr.sh: In Stop, force drvhlpr kill after 10 sec"
...
create_config_mtlk_drvhlpr()
{
	...
	host_api get_all $$ $apIndex 'gen_bd_cfg|wlan_wave300|wlan_sec|wlan_phy|wlan_vendor_wave'> $drvhlpr_params
	...
	echo "recovery_script_path = /etc/rc.d/wave_wlan_fw_recovery_notify" >> $drvhlprConfFile 

...
start_mtlk_init_hostapd()
{
	...
	/tmp/hostapd_$wlan $CONFIGS_PATH/hostapd_$wlan.conf -e /tmp/hostapd_ent_$wlan -B > /dev/console 2>/dev/console
...
create_config_mtlk_init_hostapd()
{
	...
	case $NonProcSecurityMode in
		...
		$wep)
			...
			;;
		$WPA_Personal)
			# Set the 802.11w state
			ieee80211w=$DISABLE
			if [ "$pmfEna" = "$ENABLE" ]
			then
				let ieee80211w=$pmfRequired+1
			fi
			echo "ieee80211w=$ieee80211w" >> /tmp/temp_hostapd.conf
			...
			# If WPA2-CCMP is selected and PMF is enabled and required, set SHA256 key management.
			if [ "$NonProc_WPA_Personal_Mode" = "2" ] && [ "$NonProc_WPA_Personal_Encapsulation" = "1" ] && [ "$pmfEna" = "$ENABLE" ] && [ "$pmfRequired" = "1" ]
			then
				echo "wpa_key_mgmt=WPA-PSK-SHA256" >> /tmp/temp_hostapd.conf
			else
		...
		$WPA_Enterprise)
			...
			# If WPA2-CCMP is selected and PMF is enabled and required, set SHA256 key management.
			if [ "$NonProc_WPA_Enterprise_Mode" = "2" ] && [ "$NonProc_WPA_Enterprise_Encapsulation" = "1" ] && [ "$pmfEna" = "$ENABLE" ] && [ "$pmfRequired" = "1" ]
			then
				echo "wpa_key_mgmt=WPA-EAP-SHA256" >> /tmp/temp_hostapd.conf
			else
    ...
    echo "logger_stdout=-1" >> /tmp/temp_hostapd.conf
    echo "logger_stdout_level=2" >> /tmp/temp_hostapd.conf

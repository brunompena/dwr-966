...
start_set_wls_if()
{
	...
	if [ -z "$board_mac" ]; then
		...
		echo "mtlk_set_wls_if: No MAC is defined for the platform in u-boot" >> /tmp/wave_${wlan}_start_failure
		exit 1
	fi
	...
	# Set new MAC
	api_wrapper set /sbin/ifconfig $wlan hw ether $vap_mac
        if [ $? != 0 ]
        then
                print2log ALERT "mtlk_set_wls_if: Setting MAC and starting $wlan failed."
                echo "mtlk_set_wls_if: Setting MAC and starting $wlan failed" >> /tmp/wave_${wlan}_start_failure
                exit 1
        fi
	...
	if [ "$NETWORK_TYPE" = "$STA" ]
	then
		api_wrapper set ifconfig wlan0 up
	fi
	...
	then
		...
		then
			...
			then
				...
				then
					...
					then
						...
						then
							...
							api_wrapper set iwpriv wlan0 sIpxPpaEnabled 1
				...
				api_wrapper set iwpriv $wlan sIpxPpaEnabled 1
				...
				print2log DBG "mtlk_set_wls_if.sh: iwpriv $wlan sIpxPpaEnabled 0"
				api_wrapper set iwpriv $wlan sIpxPpaEnabled 0
			fi
		fi	
	fi

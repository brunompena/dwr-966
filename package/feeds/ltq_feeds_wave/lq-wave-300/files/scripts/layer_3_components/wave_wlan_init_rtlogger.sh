...
insmod_device()
{
	...
	if [ -e $GENL_FAMILY_ID_FILE ]
	then
		mtlk_genl_family_id=`cat $GENL_FAMILY_ID_FILE`
	else
		mtlk_genl_family_id=0
	fi
	print2log DBG "wave_wlan_init_rtlogger: mtlk_genl_family_id=$mtlk_genl_family_id"
...
logger_up ()
{
	...
	ln -s $BINDIR/$fw_scd $TMPDIR/$fw_scd
	insmod_device
	mkdir -p ${DEVDIR}
	rm -f ${DEVDIR}/${device}? 
...
logger_down ()
{
	...
	[ -e $TMPDIR/$fw_scd ] && rm $TMPDIR/$fw_scd

...
led_trigger_setting()
{
    ...
    if [ "$LED_IF" != "" ]; then
        [ "$Debug" != "" ] && echo "setting LED to $LED_IF ,$LED_FS"
            echo 1 > /sys/class/leds/$LED_FS/brightness
            echo netdev > /sys/class/leds/$LED_FS/trigger
            echo $LED_IF  > /sys/class/leds/$LED_FS/device_name
            echo "link tx" > /sys/class/leds/$LED_FS/mode
            echo 125 > /sys/class/leds/$LED_FS/interval
    fi
}

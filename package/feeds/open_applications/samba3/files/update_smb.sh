...
if [ ! "$ENVLOADED" ]; then
	if [ -r /sysconfig/samba-cfg ]; then
		. /sysconfig/samba-cfg 2> /dev/null
		ENVLOADED="1"
	fi
fi
...
#	[ -L $SMB_CONF ] || ln -nsf /tmp/smb.conf $SMB_CONF
	...
	[ -z "$smb_descr" ] && return
	logger -t "[SAMBA]" -p local4.info "Clear share path..."
	echo "[global]" > $SMB_CONF
	echo "       display charset = UTF-8" >> $SMB_CONF
	echo "       unix charset = UTF-8" >> $SMB_CONF
	echo "       dos charset = cp936" >> $SMB_CONF
...
#		echo "index = $idx"
		...
		[ -z "$path" ] && continue
		echo "" >> $SMB_CONF
		logger -t "[SAMBA]" -p local4.info "Share path:$path as $name"
		echo "[$name]" >> $SMB_CONF
...
#		echo "       read only = yes" >> $SMB_CONF
		...
		if [ $rw_lvl -eq 1 ]; then
			rw_users=${users}
			logger -t "[SAMBA]" -p local4.info "read only = no"
			echo "       read only = no" >> $SMB_CONF 
		else
			ro_users=${users}
			logger -t "[SAMBA]" -p local4.info "read only = yes"
			echo "       read only = yes" >> $SMB_CONF
			 
		fi
		if [ "$users" != "all" ]; then
		        logger -t "[SAMBA]" -p local4.info "guest ok = no"
			echo "       guest ok = no" >> $SMB_CONF
		else
			logger -t "[SAMBA]" -p local4.info "guest ok = yes"
...
#			echo "       writable = Yes" >> $SMB_CONF
			logger -t "[SAMBA]" -p local4.info "valid users = $rw_users,$ro_users"
			logger -t "[SAMBA]" -p local4.info "write list = $rw_users"
			logger -t "[SAMBA]" -p local4.info "read list = $ro_users"
			echo "       valid users = $rw_users,$ro_users" >> $SMB_CONF
			echo "       write list = $rw_users" >> $SMB_CONF
			echo "       read list = $ro_users" >> $SMB_CONF
		elif [ "$rw_users" != "" -a "$rw_users" != "all" ]; then
#			echo "       writable = Yes" >> $SMB_CONF
			logger -t "[SAMBA]" -p local4.info "valid users = $rw_users"
			logger -t "[SAMBA]" -p local4.info "write list = $rw_users"
			echo "       valid users = $rw_users" >> $SMB_CONF
			echo "       write list = $rw_users" >> $SMB_CONF
		elif [ "$ro_users" != "" -a "$ro_users" != "all" ]; then
#			echo "       writable = No" >> $SMB_CONF
			logger -t "[SAMBA]" -p local4.info "valid users = $ro_users"
			logger -t "[SAMBA]" -p local4.info "read list = $ro_users" 
...
# load rc.conf
if [ ! "$ENVLOADED" ]; then
	if [ -r /sysconfig/smba-cfg ]; then
		. /sysconfig/smba-cfg 2> /dev/null
		ENVLOADED="1"
	fi
fi

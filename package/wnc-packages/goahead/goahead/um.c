 ...
 *
 *  $Id: um.c,v 1.3 2007-02-01 07:41:01 winfred Exp $
 */
 ...
 */
	...
	//ES: instead of using umconfig.txt
    umAddGroup(T("admin"), PRIV_ADMIN | PRIV_WRITE | PRIV_READ, AM_DIGEST, FALSE, FALSE);
    umAddGroup(T("user"),  PRIV_WRITE | PRIV_READ, AM_DIGEST, FALSE, FALSE);
    umAddGroup(T("guest"), PRIV_READ, AM_DIGEST, FALSE, FALSE);
	...
	if (nRows > 0) {
 	  for (index = 0; index < nRows; index++)	{
		...
		printf("[um.c]pUserName:%s, pPassword:%s, pGroup:%s\n", pUserName, pPassword, pGroup);
		printf("setup accounts...\n");
		/* STEP 2: setup to goahead um module */
        if (pUserName && strcmp(pUserName, "") && pPassword && strcmp(pPassword, "")) {
		  if (!gstrcmp(pGroup,"admin")){
		  	printf("setup accounts...Enter Admin\n");
			...
			umAddAccessLimit(T("/applications/"), AM_DIGEST, FALSE, T("admin"));
			#endif
	      }else if(!gstrcmp(pGroup,"user")){
	        printf("setup accounts...Enter User\n");
			...
			#endif
	      }else if(!gstrcmp(pGroup,"guest")){
	        printf("setup accounts...Enter Guest\n");
 ...
 */
bool_t dmfCanAccess( char *usergroup, char *group, char * userid)
{
    if( !gstrcmp(usergroup, "admin"))
    {
        return TRUE;
    }
    else if( !gstrcmp(usergroup, "user"))
    {
        if( !gstrcmp(group, "admin"))
        {
            printf("group:%s, usergroup:%s\n", group, usergroup);
            return FALSE;
        }
        else
            return TRUE;
    }
    else if( !gstrcmp(usergroup, "guest"))
    {
        if( !gstrcmp(group, "admin") || !gstrcmp(group, "user") )
        {
            printf("group:%s, usergroup:%s\n", group, usergroup);
            return FALSE;
        }
 ...
 */
    if (umGetGroupAccessMethod(usergroup) == AM_NONE) {
        printf("usergroup %s access Method is AM_NONE\n", usergroup);
        return FALSE;
    }
 ...
 */
    if (group && *group) {
       ...
       * access level required for this URL.
       */
      return dmfCanAccess(usergroup, group,user);

#else
        if (usergroup && (gstrcmp(group, usergroup) != 0)) {
            printf("GOD...group:%s, usergroup:%s\n", group, usergroup);
            return FALSE;

...
#else
static char_t   websRealm[64] = T("TG2100");   /* Realm name */
#endif
 ...
 */
            ...
            {
  						//ES: to avoid ramdisk leftspace.

  						//printf("Start to upgrade, preparing...\n");
  						//system(CGI_FW_PREPARE);
  						//printf("Start to upgrade, uploading...\n");
  						//printf("WEBS_POST Handler <%s>\n", wp->cgiStdin);
  					  fd = gopen(wp->cgiStdin, O_CREAT | O_WRONLY | O_BINARY | O_APPEND, 0666);
            }
            else
            {
              printf("Normal CGI program...\n");
 ...
 */
    ...
    if (gstrstr(url, CGI_BIN) != NULL) {
		...
		if (wp->flags & WEBS_POST_REQUEST) {
            ...
            if(gstrstr(url, CGI_UPLOADMD) == NULL)
			  wp->cgiStdin = websGetCgiCommName(wp);
			else{
			  system("/usr/local/bin/mdm_mount_ums.sh mount");
			  wp->cgiStdin = "/tmp/mdm_mass_storage/cgiWebTemp";
			}
 ...
 */
        if (code == 401) {
        //printf("401 request: message: %s;  redirect:%s \n",message,redirect);
...
static char_t* websSafeUrl(const char_t* url)
{
   ...
   spec1Count += charCount(url, '&');
   spec1Count += charCount(url, '#');
   spec1Count += charCount(url, '(');
   spec1Count += charCount(url, ')');
   spec1Count += charCount(url, '\'');
   spec2Count += charCount(url, '"');

   if (NULL != url)
   {
      safeLen = gstrlen(url);
      if (ltCount == 0 && gtCount == 0 && spec1Count == 0 && spec2Count == 0)
      {
      ...
      else
      {
         safeLen += (ltCount * 4);
         safeLen += (gtCount * 4);
		 safeLen += (spec1Count * 5);
		 safeLen += (spec2Count * 6);

...
if [ -e /tmp/bridge_lock_rmnet0 ]; then
	echo "Monitor bridge device: wan.conn is running !!!"
	return
fi
...
if [ `switch_cli IFX_ETHSW_PORT_LINK_CFG_GET nPortId=3 |grep eLink | awk '{print $2}'` == 0 ];then
	dev_num=`expr $dev_num + 1`
fi

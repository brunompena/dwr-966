...
case $1 in
 ...
 getPinConfig)
  ...
  #  echo "lteStatus.sh getPinConfig, Error: lte_get or lte_set is running. or CLIENT:Error: Get PIN Status failed, 0025."
  #fi
  if [ -n "$PIN_STATUS" ] ; then
    PIN_STATUS_1="-1"
    PIN_VERIFY_1="0"
 ...
 getWanInfo)
  ...
  IP_NETMASK_1=`grep 'CLIENT:netmask:' $WANINFO | awk -F ':' '{print $3}'`	
  if [ -n "$IP_ADDR_1" ] ; then
  ...
  if [ -n "$IP_NETMASK_1" ] ; then
    echo "$IP_NETMASK_1" >> $WANINFO
  else
    echo "0.0.0.0" >> $WANINFO
  fi
  ;;

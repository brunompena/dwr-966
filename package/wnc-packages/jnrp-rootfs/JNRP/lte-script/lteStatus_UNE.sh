...
case $1 in
 getAPNProfiles)
  ...
  `lte_get -i 0 3 --timeout 5000 > $APNPROFILE`
  PROFILE_NAME_3=`grep 'CLIENT:Profile Name' $APNPROFILE | awk -F '=' '{print $2}'`
  APN_NAME_3=`grep 'CLIENT:APN Name' $APNPROFILE | awk -F '=' '{print $2}'`
  ...
  if [ -z "$PROFILE_NAME_3" ] ; then
    PROFILE_NAME_3=`grep 'config.lteNwProfile\[3\]\["name"\]' $CUR_CONFIG_PATH | awk -F '= ' '{print $2}' | cut -d'"' -f2`
  fi
  if [ -z "$APN_NAME_3" ] ; then
    APN_NAME_3=`grep 'config.lteNwProfile\[3\]\["APNName"\]' $CUR_CONFIG_PATH | awk -F '= ' '{print $2}' | cut -d'"' -f2`
  fi
  echo -e "$PROFILE_NAME_1" > $APNPROFILE
  echo -e "$APN_NAME_1" >> $APNPROFILE
  echo -e "$PROFILE_NAME_2" >> $APNPROFILE
  echo -e "$APN_NAME_2" >> $APNPROFILE
  echo -e "$PROFILE_NAME_3" >> $APNPROFILE
  echo -e "$APN_NAME_3" >> $APNPROFILE
  ;;
 getAPNStatus)
  ...
  IP_STATUS_3=`lte_get --ip-status -q 2 --timeout 5000 | grep 'CLIENT:status:' | awk -F ':' '{print $3}'`
  ...
  if [ -z "$IP_STATUS_3" ] ; then
    IP_STATUS_3=`grep 'config.lteIPStatus\[3\]\["status"\]' $CUR_CONFIG_PATH | awk -F '= ' '{print $2}' | cut -d'"' -f2`
  fi
  echo -e "$IP_STATUS_1" > $APNSTATUS
  echo -e "$IP_STATUS_2" >> $APNSTATUS
  echo -e "$IP_STATUS_3" >> $APNSTATUS
  ;;
 getPinConfig)

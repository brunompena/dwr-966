/* adpSnPrintf.c - snprintf replacement */
...
--------------------
01e,09may08,san  added #undef MAX for fixing compiler warning.
01d,10oct07,san  added CONFIG_NO_LONG_LONG.
01c,20feb06,rks  renamed round to adpRound to avoid conflict with other
                 definition in one of the header file in target/h.
01b,02oct03,ksn  moved to adaptos layer.
01a,06jul02,ksn  fixed for compilers with no long long support
*/
...
/* Added for fixing integrity warning 'incompatible redefinition of macro */
#undef MAX
...
#if defined(NO_LONG_LONG) || defined(CONFIG_NO_LONG_LONG) 
#define HAVE_64BIT_BROKEN
#endif /* NO_LONG_LONG || CONFIG_NO_LONG_LONG */
...
#ifndef HAVE_64BIT_BROKEN
                        else if (cflags == DP_C_LONG_LONG)
                            value = va_arg (args, long long);
#endif /* HAVE_64BIT_BROKEN */
                        else
                            value = va_arg (args, int);
      ...
      int min, int max, int flags)
{
     ...
     */
    fracpart = adpRound((pow10 (max)) * (ufvalue - intpart));

...
soap_send_raw(struct soap *soap, const char *s, size_t n)
{
	...
	if (soap->mode & SOAP_IO) { 
		...
		while (n >= i) { 
			...
			if (soap_flush(soap)) return soap->error;
			s += i;
			n -= i;
			i = SOAP_BUFLEN;
		}
...
#ifdef WITH_GZIP
			soap->z_crc = crc32(soap->z_crc, (Byte*)soap->buf, 
								(unsigned int)n);
#endif
			do { 
...
soap_match_namespace(struct soap *soap, const char *id1, const char *id2, size_t n1, size_t n2) 
{
	...
	if (np) { 
		/* K.H. Wu add for ACS format and avoid error in the initial stage */
		if (!(strncmp(np->id, id1, n1)||np->id[n1]))
			return SOAP_OK; 
		if (np->index < 0
...
soap_match_tag(struct soap *soap, const char *tag1, const char *tag2)
{
	...
	if (t) { 
		if (s) { 
			if (t[1] && (err=SOAP_STRCMP(s + 1, t + 1)))
				return SOAP_TAG_MISMATCH;
			if (t != tag2 && (err = soap_match_namespace
				(soap, tag1, tag2, s - tag1, t - tag2))) {
			...
			return SOAP_TAG_MISMATCH;
		} else if (t != tag2 && (err = soap_match_namespace
					(soap, tag1, tag2, 0, t - tag2))) { 
			DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Tags '%s' and '%s' match but namespaces differ\n", tag1, tag2));
			if (err == SOAP_NAMESPACE)
				return SOAP_TAG_MISMATCH;
			return err;
	...
	if (s) { 
		if (SOAP_STRCMP(s + 1, tag2)) return SOAP_TAG_MISMATCH;
...
soap_match_array(struct soap *soap, const char *type)
{
	...
	if (strncmp(soap->arrayType, "string", 6) == 0) // =-=- Liang.Y.S.@2008.04.01 -=-=
		return SOAP_OK;
	if (strncmp(soap->arrayType, "xsd:string", 10) == 0) // =-=- Liang.Y.S.@2008.05.08 -=-=
		return SOAP_OK;
	//---Alvin add
	if (strstr(soap->arrayType, ":string") != NULL)	
		return SOAP_OK;
	//---
  ...
  if (*soap->arrayType)
    if (soap_match_tag(soap, soap->arrayType, type)
     ...
     && soap_match_tag(soap, soap->arrayType, "string") // =-=- Liang.Y.S.@2007.11.08 -=-=
    )
...
again:
...
#endif
#ifdef _JPR_AM_
	if (soap->bind_device != NULL) {
		struct ifreq ifr;
		memset(&ifr,0,sizeof(ifr));
		snprintf(ifr.ifr_name,sizeof(ifr.ifr_name),soap->bind_device);
	...
	else if (soap->connect_flags && setsockopt(fd, SOL_SOCKET, soap->connect_flags, 
							(char*)&set, sizeof(int))) { 
		soap->errnum = soap_socket_errno(fd);
	...
	if ((soap->keep_alive || soap->tcp_keep_alive) && 
		setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (char*)&set, sizeof(int))) { 
		soap->errnum = soap_socket_errno(fd);
...
#ifdef TCP_KEEPIDLE
	if (soap->tcp_keep_idle && setsockopt((SOAP_SOCKET)fd, IPPROTO_TCP, 
		TCP_KEEPIDLE, (unsigned int*)&(soap->tcp_keep_idle), sizeof(int))) { 
		soap->errnum = soap_socket_errno(fd);
...
#endif
#ifdef TCP_KEEPINTVL
	if (soap->tcp_keep_intvl && setsockopt((SOAP_SOCKET)fd, IPPROTO_TCP, 
		TCP_KEEPINTVL, (unsigned int*)&(soap->tcp_keep_intvl), sizeof(int))) { 
		soap->errnum = soap_socket_errno(fd);
...
#endif
#ifdef TCP_KEEPCNT
	if (soap->tcp_keep_cnt && setsockopt((SOAP_SOCKET)fd, IPPROTO_TCP, 
		TCP_KEEPCNT, (unsigned int*)&(soap->tcp_keep_cnt), sizeof(int))) { 
		soap->errnum = soap_socket_errno(fd);
...
#endif
#ifdef TCP_NODELAY
	if (!(soap->omode & SOAP_IO_UDP) && setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, 
							(char*)&set, sizeof(int))) { 
		soap->errnum = soap_socket_errno(fd);
	...
	for (;;) { 
		...
		{ 
			...
			else if (soap->connect_timeout && (err == SOAP_EINPROGRESS || 
		...
		/* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
				if (!getsockopt(fd, SOL_SOCKET, SO_ERROR, 
					(char*)&soap->errnum, &k) && !soap->errnum)	
					break;
				DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Could not connect to host\n"));
				if (!soap->errnum) soap->errnum = soap_socket_errno(fd);
				soap_set_sender_error(soap, tcp_error(soap), "connect failed in tcp_connect()", SOAP_TCP_ERROR);
				soap->fclosesocket(soap, fd);
#ifdef WITH_IPV6
				freeaddrinfo(ressave);
	...
	if (!soap_tag_cmp(endpoint, "https:*")) {
		...
		if (soap->proxy_host) { 
			...
			if (soap_begin_send(soap) || (soap->error = 
					soap->fposthdr(soap, soap->tmpbuf, NULL))) { 
				soap->fclosesocket(soap, fd);
				return SOAP_INVALID_SOCKET;
			}
#ifndef WITH_LEAN
				...
				if (ext_count > 0) { 
					...
					for (i = 0; i < ext_count; i++) { 
						...
						if (ext_str && !strcmp(ext_str, "subjectAltName")) { 
							//---X509V3_EXT_METHOD *meth = X509V3_EXT_get(ext);   by Alvin
				...
				*/
				...
				if (!ok && (subj = X509_get_subject_name(peer))) { 
					...
					do { 
						...
						LOG(LOG_DEBUG, "Certificate name=%s Connection host=%s",name->data,host);
						if (name) { 
...
//---Alvin add for basic & digest auth.  2012/2/4
void soap_restore_post_parsehdr(struct soap* soap)
{
	soap->fposthdr = http_post_header;
	soap->fparsehdr = http_parse_header;
}
...
{ 
	...
	*ppp = pp = &soap->pblk->plist[soap->pidx++];
	if (a) h = soap_hash_ptr(a->__ptr);
	else h = soap_hash_ptr(p);
...
{ 
  if (!strncmp(value, "SOAP-ENC:Array",14))  //  =-=- Liang Y.S.@2007.11.21 -=-=
	return SOAP_OK;
#ifdef WITH_DOM
    ...
    { 
	 ...
	 || (soap->mode & SOAP_ENC_MTOM))
      ...
      { if (!soap_match_tag(soap, tp->name, "SOAP-ENC:arrayType"))
	    ...
	    //---Alvin add
		else if (strstr(tp->name, "arrayType") != 0)
        { 
			s = soap_strrchr(tp->value, '[');
			if (s && (size_t)(s - tp->value) < sizeof(soap->arrayType))
			{ 
		...
		/* K.H. Wu add for ACS format */
        else if (!soap_match_tag(soap, tp->name, "env:mustUnderstand")
              && (!strcmp(tp->value, "1") || !strcmp(tp->value, "true")))
          soap->mustUnderstand = 1;
    ...
    { 
      ...
      { if (m > 0)
        ...
        default:
          ...
          if (soap->mode & SOAP_C_MBSTRING)
          { 
	    LOG(LOG_DEBUG, "into wctomb(1) but do nothing!!\n");
...
{ 
	...
	if ((soap->mode & SOAP_ENC_DIME) && !(soap->mode & SOAP_ENC_MTOM)) { 
		...
		for (content = soap->dime.first; content; 
			...
			if (content->options)
				count += ((((unsigned char)
					content->options[2] << 8) | 
				((unsigned char)content->options[3]))+7)&(~3);
	...
	if ((soap->mode & SOAP_ENC_MIME) && soap->mime.boundary) { 
		...
		for (content = soap->mime.first; content; 
					content = content->next) { 
			register const char *s;
			/* count \r\n--boundary\r\n */
			count += 6 + n;
...
{ struct soap_cookie *p = NULL, *q;
  ...
  { s = soap_decode_key(tmp, sizeof(tmp), s);
    ...
    { 
	  //---Alvin add for cookies isseue
	  if (!soap_tag_cmp(tmp, "Discard"))
		  continue;
	  //---
	  if (p)
...
{ 
	...
	if (!(soap->mode & SOAP_IO_KEEPALIVE)) soap->keep_alive = 0;
	soap->ahead = 0;
	soap->peeked = 0;
	soap->level = 0;
	soap->part = SOAP_BEGIN;
	...
	*soap->endpoint = '\0';
...
#endif
#ifndef WITH_LEANER
	if (soap->fprepareinit) soap->fprepareinit(soap);
#endif
	c = soap_getchar(soap);
#ifdef WITH_GZIP
	if (c == 0x1F) {
	...
	if ((int)c == EOF) return soap->error = SOAP_EOF;
	soap_unget(soap, c);
#ifndef WITH_NOHTTP
	/* if not XML or (start of)BOM or MIME/DIME/ZLIB, assume HTTP header */
	if (c != '<' && c != 0xEF && !(soap->mode & (SOAP_ENC_MIME | 
						SOAP_ENC_DIME | SOAP_ENC_ZLIB))) { 
		soap->mode &= ~SOAP_IO;
		...
		if (soap->error == SOAP_STOP) return soap->error;
	...
	if (soap_get0(soap) == (int)EOF) { 
		if (soap->status == 200) return soap->error = SOAP_NO_DATA;
	...
	if (soap->zlib_in != SOAP_ZLIB_NONE) {
		...
		if (soap->zlib_in != SOAP_ZLIB_DEFLATE) { 
			c = soap_get1(soap);
			if (c == 0x1F) { 
				if (soap_getgziphdr(soap)) return soap->error;
				if (inflateInit2(&soap->d_stream, -MAX_WBITS) != Z_OK)
					return soap->error = SOAP_ZLIB_ERROR;
				soap->z_crc = crc32(0L, NULL, 0);
				DBGLOG(TEST, SOAP_MESSAGE(fdebug, "gzip initialized\n"));
...
#ifndef WITH_LEANER
	if (soap->mode & SOAP_ENC_MIME) { 
		if (soap_getmimehdr(soap)) return soap->error;
		if (soap->mime.start) { 
			do
			{ 
				if (!soap->mime.last->id) break;
...
{ char header[SOAP_HDRLEN], *s;
  ...
  *soap->endpoint = '\0';
  ...
  soap->http_content = NULL;
  soap->status = 0;*/  //2012.6.21 by Alvin
  do
  { if (soap_getline(soap, soap->msgbuf, sizeof(soap->msgbuf)))
...
soap_envelope_begin_out(struct soap *soap)
{
	...
	size_t n = 0;
	if ((soap->mode & SOAP_ENC_MIME) && soap->mime.boundary && 
		soap->mime.start && strlen(soap->mime.boundary) + 
		strlen(soap->mime.start) < sizeof(soap->tmpbuf) - 80 ) { 
		const char *s;
		if ((soap->mode & SOAP_ENC_DIME) && !(soap->mode & SOAP_ENC_MTOM))
		s = "application/dime";
		else if (soap->version == 2) { 
...
soap_envelope_begin_in(struct soap *soap)
{
	register struct Namespace *p;
	soap->part = SOAP_IN_ENVELOPE;
	/* K.H. Wu add for our fucking ACS header. */
	if (soap_element_begin_in(soap, "SOAP-ENV:Envelope", 0, NULL) &&
	    soap_element_begin_in(soap, "env:Envelope", 0, NULL)) { 
#ifndef WITH_LEAN
	...
	if (p) { 
		...
		if (!strcmp(ns, soap_env1)) { 
			...
			if ((p[1].out = (char*)SOAP_MALLOC(soap, 
						sizeof(soap_enc1))))
...
{ 
	int retcode;
	soap->part = SOAP_IN_BODY;
	if ((retcode=soap_element_begin_in(soap, "SOAP-ENV:Body", 0, NULL))){
		/* K.H. Wu add for our fucking ACS format */
		if ((retcode=soap_element_begin_in(soap, "env:Body", 0, NULL))){
			return soap->error;
		}
	}
	if (!soap->body) soap->part = SOAP_NO_BODY;
	return SOAP_OK;
}
#endif
...
soap_recv_header(struct soap *soap)
{
	...
	//K.H. Wu: In order to ignor  SOAP_MUSTUNDERSTAND e
	if (soap->error == SOAP_MUSTUNDERSTAND) soap->error = SOAP_OK;
	return soap->error;
}
#endif
...
soap_connect_command(struct soap *soap, int http_command, const char *endpoint, const char *action)
{
	...
	if (soap->fconnect){ 
		if ((soap->error = 
			soap->fconnect(soap, endpoint, soap->host, soap->port)))
			return soap->error;
	} else
#endif
	...
	if (soap_begin_send(soap)) return soap->error;
	...
	if ((soap->mode & SOAP_IO) != SOAP_IO_STORE && 
			!(soap->mode & SOAP_ENC_XML) && endpoint) { 
		unsigned int k = soap->mode;
		soap->mode &= ~(SOAP_IO | SOAP_ENC_ZLIB);
		if ((k & SOAP_IO) != SOAP_IO_FLUSH)
		soap->mode |= SOAP_IO_BUFFER;
		if ((soap->error = soap->fpost(soap, endpoint, soap->host, 
					soap->port, soap->path, action, count)))
			return soap->error;
#ifndef WITH_LEANER
	...
	if (http_command != SOAP_POST) return soap_end_send(soap);
#endif
	return SOAP_OK;
}
#endif
...
http_post(struct soap *soap, const char *endpoint, const char *host, int port, const char *path, const char *action, size_t count)
{
	register const char *s;
	register int err;
	if (soap->status == SOAP_GET) s = "GET";
	else s = "POST";
#ifdef PALM
	if (!endpoint || (soap_tag_cmp(endpoint, "http:*") && 
		soap_tag_cmp(endpoint, "https:*") && 
		strncmp(endpoint, "httpg:", 6)) && 
		strncmp(endpoint, "_beam:", 6) && 
		strncmp(endpoint, "_local:", 7) && 
		strncmp(endpoint, "_btobex:", 8))
#else
	if (!endpoint || (soap_tag_cmp(endpoint, "http:*") && 
		soap_tag_cmp(endpoint, "https:*") && strncmp(endpoint, "httpg:", 6)))
#endif
		return SOAP_OK;
	...
	if (soap->status != SOAP_GET && (soap->version == 1 || 
		(action && *action && strlen(action) < sizeof(soap->tmpbuf) - 2))) {
...
{ 
	...
	if (!(soap->omode & (SOAP_ENC_XML | 
			SOAP_IO_STORE /* this tests for chunking too */))
...
{ register int status = soap->error;
  ...
  { soap->error = SOAP_OK;
    ...
    {     
       if (soap_connect(soap, MGT_SERVER_URL, "")
        || soap_envelope_begin_out(soap)
        || soap_putheader(soap)
        || soap_body_begin_out(soap)
        || soap_putfault(soap)
...
{ if (soap_check_state(soap))
  ...
  { const char *c, *v = NULL, *s, **d;
	...
	LOG(LOG_DEBUG, "[SOAP-Fault] %s%d fault: %s [%s]\n", soap->version ? "SOAP 1." : "Error ", soap->version ? (int)soap->version : soap->error, c, v ? v : "no subcode");
	LOG(LOG_DEBUG, "[SOAP-Fault] %s\n", s ? s : "[no reason]");
	LOG(LOG_DEBUG, "[SOAP-Fault] Detail: %s\n", d && *d ? *d : "[no detail]");
  }
}
#endif
#endif

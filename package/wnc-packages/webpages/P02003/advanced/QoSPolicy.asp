...
function SessionValidate()
{
	...
	var i18n_enterValidSession = gettext("Invalid Value. The Limit Sessions cannot be empty.");
...
function checkBWApply()
{
  ...
  document.BandwidthManageForm.action="/goform/setBandWidthManage?<%getUniName();%>=<%getUniId();%>";
...
<!-- Session Limit -->
  ...
  <form name="sessionLimitForm" method="post" action="/goform/setSessionLimit?<%getUniName();%>=<%getUniId();%>">
...
<!-- Bandwidth Profiles-->	
  ...
  <div class="secInfo">
    <br><script>document.write(gettext("The packet would be set the priority accroding to the 'Rule' content. If more than one rule content matched, would be set the latest matched rule. To every packet, only one rule is effective at the same time."));</script>
	<br>
  </div>
  <form name="QoSPolicyForm" method="post" action="/goform/setQoSTable?<%getUniName();%>=<%getUniId();%>"> 

...
function pageValidate ()
{
	if(!CheckLoginInfo())
		return false;
	if (numericValueRangeCheck (document.getElementById ('txtport1'), '', '', 1, 65535, true, gettext("Invalid Port Range."), '') == false)
		return false;
	var ipAddrObj = document.getElementById('txtIPAddr');
	if (checkIpAddr (ipAddrObj, gettext("IP"), gettext("Please enter valid IP Address")) == false ) return false;
...
function checkIpAddr (thisObj, ipStr, alertStr)
{
	...
	var i18n_forOctet = gettext("for octet ");
	var i18n_invalidSubnet = gettext("Invalid Subnet Mask.");
...
<div class="contentBg" id="sub_forwarding">
	...
	<div class="secBg">
		...
		<div class="secInfo">
			<br><script>document.write(gettext("This page allows you to to use a custom port for services run on local network computers. The rules configured here allow this service to bypass the default security mode."));</script>
			<br>
		</div>
		<form name="port_fwd" method="post" action="/goform/setPortFwd?<%getUniName();%>=<%getUniId();%>">
...
<!--				onkeypress="return numericValueCheck (event, 'ABCDEFabcdef:.')" onkeydown="if (event.keyCode == 9) {return checkIpAddr (this, 'IP', '$| trStrings["20216"] or 'i18nHTMLMissing' |$'); }">-->
			...
			<tr>
				<td><script>document.write(gettext("Enable Port Translation"));</script></td>
				<td><input type="checkbox" name="enable_port_translation"  id="chkEnbPort" value="1" onclick="return chkEnbport();">
				<td></td>
			</tr>
			<tr>
				<td><script>document.write(gettext("Port"));</script></td>
...
<!--				onkeypress="return numericValueCheck (event)" onkeydown="if (event.keyCode == 9) {return numericValueRangeCheck (this, '', '', 1, 65535, true, '$| trStrings["20111"] or 'i18nHTMLMissing' |$:', '');}" /></td>-->
		...
		<div class="submitBg">
			<input type="submit" id="port_apply" value="Apply" class="submit" title="Apply" onmouseover="this.className = 'submitHover'" onmouseout="this.className = 'submit'" onclick="return pageValidate ();">
			<input type="button" id="port_reset" value="Reset" class="submit" title="Reset" onclick="doRedirect();" onmouseover="this.className = 'submitHover'" onmouseout="this.className = 'submit'">
		</div>
		</form>
	</div>
	<!-- Section End -->
...
<script type="text/javascript">
	document.getElementById('port_apply').value=gettext("Apply");
	document.getElementById('port_reset').value=gettext("Reset");

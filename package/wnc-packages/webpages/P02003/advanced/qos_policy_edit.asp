...
function qos_edit_validate()
{
    ...
    txtFieldIdArr[0] = "selProfile," + gettext("Please add profile first..");
    txtFieldIdArr[1] = "txtIpAddr," + gettext("Please enter a valid IP Address");
    txtFieldIdArr[2] = "txtMacAddr," + gettext("Please enter a valid MAC address");
	...
	if (ipv4Validate ('qosIP', gettext("IP"), false, true, gettext("Invalid IP Address."), gettext("for octet "), true) == false){
      	return false;
	}  

	var dscpObj = document.getElementById('qosDSCP');
	if (!dscpObj.disabled){
		if(numericValueRangeCheck (dscpObj, "", gettext("DSCP"), 0, 63, true, gettext("Invalid DSCP value:"), "") == false){
			return false;
		}	
	}
...
if (QosEditInfo == ""){  //Add	
	...
	for (var i=0; i < serviceNameArr.length; i++)
    {
  	...
  	if (count > 0){
  	  alert(gettext("The Service is duplicate."));
...
<div class="contentBg">
  ...
  <form method="post" action="/goform/setQosEdit?<%getUniName();%>=<%getUniId();%>">
  ...
  <div style="display:block;" id="qos_add" name="qos_add" style="margin:10px 0 0 15px;">
    ...
    <table class="configTbl" cellspacing="0">
	  ...
	  <tr>
		...
		<td><input type="text" name="qosDSCP" value="" size="20" class="configF1" id="qosDSCP" onkeypress="return numericValueCheck (event)" onkeydown="if (event.keyCode == 9) {return numericValueRangeCheck (this, '', '', 1, 4096, true, gettext('Invalid DSCP value:'), '');}"></td>
	  </tr>
    </table>      
    <div>

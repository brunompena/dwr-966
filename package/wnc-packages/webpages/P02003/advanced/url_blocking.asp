	...
	for (var i=0; i < objArr.length; i++)
	{
		if (objArr[i].type == 'checkbox' && objArr[i].name!="imgSelectAllChk")
		{
			if (objArr[i].checked==true)
			{
				...
				if (checkFieldExist("tblParentCtrlList","profile_fltr_name_",fltr_name,action,gettext("Filter Name"),gettext("Parental Control Service")))
					return false;
			}
		}
	}
...
<div class="contentBg">
	...
	<div class="secBg">	
		...
		<div class="secInfo">
			<br><script>document.write(gettext("This security tool allows you to block access to Internet domains that contain certain defined keywords. Up to 32 key words to check in a domain can be configured in a single filtering policy."));</script>
			<br>
		</div>
		<form method="post" action="/goform/setFltrTable?<%getUniName();%>=<%getUniId();%>">
		...
		<table border="0" cellpadding="0" cellspacing="0" id="tblUrlBlock" width="500px" style="table-layout: fixed;" class="specTbl">
			<tr>
				...
				<td class="tdH" width="30%"><script>document.write(gettext("Filter Name"));</script></td>
				<td class="tdH" width="30%"><script>document.write(gettext("Keywords"));</script></td>
				<td class="tdH" width="30%"><script>document.write(gettext("Domains"));</script></td>
			</tr>			  
		...
		<div class="submitBg">
			<table border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td><input type="submit" id="url_add" value="Add" class="tblbtn" title="Add" name="button.add.ContentFltr.urlBlockingConfig.-1"  onmouseover="this.className = 'tblbtnHover'" onmouseout="this.className = 'tblbtn'" onclick="return fltr_list_action('add');"></td>
					<td><input type="submit" id="url_edit" value="Edit" class="tblbtn" title="Edit" name="button.edit.ContentFltr.urlBlockingConfig"  onmouseover="this.className = 'tblbtnHover'" onmouseout="this.className = 'tblbtn'" onclick="return fltr_list_action('edit');"></td>
					<td><input type="submit" id="url_delete" value="Delete" class="tblbtn" title="Delete" name="button.delete.ContentFltr.urlBlocking"  onmouseover="this.className = 'tblbtnHover'" onmouseout="this.className = 'tblbtn'" onclick="return fltr_list_action('delete');"></td>
				</tr>
			</table>
		</div>
		</form>
...
<script type="text/javascript">
	document.getElementById('url_add').value=gettext("Add");
	document.getElementById('url_edit').value=gettext("Edit");
	document.getElementById('url_delete').value=gettext("Delete");
</script>
</body>
</html>

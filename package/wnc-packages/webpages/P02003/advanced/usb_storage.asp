...
{	
	...
	if (action=="add")
	{
		if (objArr.length > 5)
		{
			alert(gettext("There are already 5 file share."));
			ret=false;			
		}
		else
...
function loadUsbPage()
{
	...
	if (dlna_status)
	{
		infoMsg=gettext("This page displays the USB storage information.");
		document.getElementById('usbUI').style.display = "block";
	}
	else
	{
		infoMsg=gettext("This page displays the USB storage information when DLNA Media Server is enabled.");
		document.getElementById('usbUI').style.display = "none";	
	}
	document.getElementById('secInfo').innerHTML=infoMsg;
...
{
  var pwd = get_by_id("SAMBA_PASSWORD").value;
  if(checked == true){  	
    get_by_id("SAMBA_PASSWORD").outerHTML=
		'<input type="text" name="SAMBA_PASSWORD" class="configF1" value="" id="SAMBA_PASSWORD" size="12"  maxlength="24" >';	
  } else {
    get_by_id("SAMBA_PASSWORD").outerHTML=
		'<input type="password" name="SAMBA_PASSWORD" class="configF1" value="" id="SAMBA_PASSWORD" size="12"  maxlength="24" >';	
  }
  get_by_id("SAMBA_PASSWORD").value = pwd;
}
function pageValidate()
{
	...
	if(get_by_id("SAMBA_NAME").value.length > 15)
	{
		alert(gettext("The maximum length of Samba Server Name is 15"));
		return false;
	}	
...
<div class="contentBg">
<div class="secH1"><script>document.write(gettext("USB Storage"));</script></div>
	<!-- Section Begin -->
	<div class="secBg">
	...
	<table id="usbUI" border="0" cellpadding="0" cellspacing="0" style="display:none">
	 <tr>
		<td class="tdH" width="25%"><script>document.write(gettext("Directory"));</script></td>
		<td class="tdH" width="15%"><script>document.write(gettext("File System"));</script></td>
		<td class="tdH" width="15%"><script>document.write(gettext("Total"));</script></td>
		<td class="tdH" width="15%"><script>document.write(gettext("Used"));</script></td>
		<td class="tdH" width="15%"><script>document.write(gettext("Available"));</script></td>
		<td class="tdH" width="15%"><script>document.write(gettext("Percent"));</script></td>
	 </tr>
	...
	<!-- Samba Setting -->
	<div class="secBg">
	<div class="secInfo"><script>document.write(gettext("SAMBA Server Settings"));</script></div>
		<form action="/goform/setSambaSetting?<%getUniName();%>=<%getUniId();%>" method="post">	
		<table class="configTbl" cellspacing="0" cellpadding="0" border="0">		
			<tr>
				<td><script>document.write(gettext("Enable SAMBA"));</script></td>
				<td>
			...
			<tr>
				<td><script>document.write(gettext("User Password"));</script></td>
				<td>
					<input type="password" name="SAMBA_PASSWORD" class="configF1" value="" id="SAMBA_PASSWORD" size="12"  maxlength="24" >
					<span class="spanText"><script>document.write(gettext("(User Account \"all\" does not require a password)"));</script></span>
				</td>
				<td>&nbsp;</td>
			</tr>				
			<tr>
				<td><script>document.write(gettext("Display Password"));</script></td>
				<td>
					<input type="checkbox" id="PWD_CHK" name="PWD_CHK" onclick="show_password(this.checked);">					
				</td>
				<td>&nbsp;</td>
			</tr>
		</table>	
		<div class="submitBg">
			<input type="submit" id="samba_apply" value="Apply" class="submit" onclick="return pageValidate();" onmouseover="this.className = 'submitHover'" onmouseout="this.className = 'submit'">			
			<input type="button" id="samba_reset" value="Reset" class="submit" title="Reset" onclick="doRedirect();" onmouseover="this.className = 'submitHover'" onmouseout="this.className = 'submit'">
		</div>	
		</form>
	</div>
	...
	<!-- Samba share table -->	
	<div class="secBg">
	<div class="secInfo"><script>document.write(gettext("Create a sharing profile."));</script></div>
	<form action="/goform/setSambaShare?<%getUniName();%>=<%getUniId();%>" method="post">
		...
		<table id="tblSambaShare" border="0" cellpadding="0" cellspacing="0">
			<tr>		 
				...
				<td class="tdH" width="20%"><script>document.write(gettext("Sharing Profile Name"));</script></td>
				<td class="tdH" width="35%"><script>document.write(gettext("Folder Path"));</script></td>
				<td class="tdH" width="25%"><script>document.write(gettext("Access Level"));</script></td>
				<td class="tdH" width="15%"><script>document.write(gettext("Users"));</script></td>
			</tr>		
...
<script type="text/javascript">
	...
	document.getElementById('sambaShareAdd').value=gettext("Add");
	document.getElementById('sambaShareEdit').value=gettext("Edit");
	document.getElementById('sambaShareDel').value=gettext("Delete");
</script>
</body>
</html>

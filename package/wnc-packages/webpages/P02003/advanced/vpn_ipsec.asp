...
function nat_validate()
{
	if (get_by_id("keep_alive").disabled==false)
	{
		...
		{
			alert("Enter valid Keep Alive time");
			get_by_id("statusMsg").innerHTML="Enter valid Keep Alive time";
			return false;
		}
	}
	return true;
...
<div class="contentBg">
	...
	<form method="post" action="/goform/setVpnNAT?<%getUniName();%>=<%getUniId();%>">
	...
	<form method="post" action="/goform/setVpnIPSecTable?<%getUniName();%>=<%getUniId();%>">

...
function onClickScan()
{
	if(!CheckLoginInfo())
		return false;
   	 var msg = gettext("Suggest to disconnect internet first, otherwise the scanning may be incorrect.");
   	 if(confirm(msg)) {
...
<div class="contentBg">
	<div class="secH1"><script>document.write(gettext("Network Selection"));</script></div>
	<div class="secBg">
		...
		<form name="NETWORK_SCAN" action="/goform/setNetworkSelMode?<%getUniName();%>=<%getUniId();%>" method="post">
		<table cellspacing="0" class="configTbl">
			<tr>
    	<td width="230"><script>document.write(gettext("LTE Network Selection Method"));</script></td>
    	<td width="95"><input type="radio" name="providerRegMethod" id="networkModeAuto" value="auto" checked onclick="changeRegType('auto');"/><script>document.write(gettext("Auto"));</script></td>
  	  ...
  	  <tr><td><iframe src="../transparent.htm" name="show_isp_list_frame" id="show_isp_list_frame" height="400" width="550" frameborder="0" scrolling="yes" allowTransparency="true" style="display:none;"></iframe></td></tr>
		</table>

...
function CheckLoginInfo(state)
{
	...
	if(startTime.dateDiff("s",currentTime) > idle_time)
	{
		alert(gettext("You have been logged out as a result of being inactive for ")+idle_time+gettext(" seconds.")+"\n"+
...
function CheckInitLoginInfo(login_info)
{
	...
	if((original_time.dateDiff("s",now_time) > idle_time) && (original_time.dateDiff("y",now_time)<2))
	{
		alert(gettext("You have been logged out as a result of being inactive for ")+idle_time+gettext(" seconds.")+"\n"+
			gettext("Please login again then you can access the Web GUI."));
		window.location.href="../login.asp";

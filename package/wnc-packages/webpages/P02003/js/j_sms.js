...
var sms_inbox_list = new Array();
var sms_outbox_list = new Array();
var sim_sms_list = new Array();
var sms_draft_list = new Array();
...
var temp_inbox_count=0;
var temp_sim_sms_count=0;
var temp_draft_max_count=0;
var temp_outbox_max_count=0;
var temp_inbox_max_count=0;
var temp_sim_sms_max_count=0;
...
function sms_draft_send()
{
	...
	if(temp_outbox_count >= 100)
	{
		alert(gettext("There are already 100 Outbox."));
		return false;
	}			
...
function GetWWANPinStatus(pin_status)
{
	if(typeof(pin_status.errno) != "undefined" && pin_status.errno != 0)
	{
		alert(gettext(pin_status.errmsg));
	}
	else
	{		
		if (pin_status.card_status == 0)
			sim_status = "not inserted";
		else if (pin_status.card_status == 1)
			sim_status = "inserted";
		else 
			sim_status = "error";

		if (pin_status.pin1.status == 0)
			pin1_status = "not initialized";
		else if (pin_status.pin1.status == 1)
			pin1_status = "enabled, not verified";
		else if (pin_status.pin1.status == 2)
			pin1_status = "enabled, verified";
		else if (pin_status.pin1.status == 3)
			pin1_status = "disabled";
		else if (pin_status.pin1.status == 4)
			pin1_status = "blocked";
		else if (pin_status.pin1.status == 5)
			pin1_status = "permanently blocked";
...
function GetSMSInfo(sms_info)
{
	if(typeof(sms_info.errno) != "undefined" && sms_info.errno != 0)
	{
		alert(gettext(sms_info.errmsg));
	}
	else
	{
		temp_draft_count=sms_info.draft_count;
		temp_outbox_count=sms_info.outbox_count;	
		temp_inbox_count = sms_info.inbox_count;
		temp_sim_sms_count = sms_info.sim_count;
		temp_draft_max_count = sms_info.draft_max;
		temp_outbox_max_count = sms_info.outbox_max;
		temp_inbox_max_count = sms_info.inbox_max;
		temp_sim_sms_max_count = sms_info.sim_max;
		/*
		if ($("#get_sim_sms_info").length > 0)
		{
		...
		*/
		...
		if(sms_flag == "inbox"){
			LoadSMSInbox();
		}else if(sms_flag == "outbox"){
			LoadSMSOutbox();
		}else if(sms_flag == "draft"){
			LoadSMSDraft();
		}else if(sms_flag == "sim_sms"){
			LoadSIMSMS();
		}
...
function GetSMSInboxInfoCB(msg)
{
	if(msg.get_sms_inbox_records.errno == 0){
		var inboxLength = msg.get_sms_inbox_records.records.length;
		if ($("#sms_inbox_table").length > 0){
			if (inboxLength > 0){
				for (var i = 0; i < inboxLength; i++){
		 			var temp_inbox = new Object();
					temp_inbox.new_sms = msg.get_sms_inbox_records.records[i].new_sms;
					temp_inbox.name = msg.get_sms_inbox_records.records[i].name;
					temp_inbox.number = msg.get_sms_inbox_records.records[i].number;
					temp_inbox.time = msg.get_sms_inbox_records.records[i].time;
					temp_inbox.content = msg.get_sms_inbox_records.records[i].content;
					temp_inbox.idx = msg.get_sms_inbox_records.records[i].idx;
					sms_inbox_list.push(temp_inbox);
				}
			}
			if(sms_inbox_list.length == temp_inbox_count){
				$("#short_inbox_total_num").text(sms_inbox_list.length);
				if (sms_inbox_list.length >= 100)
				{
					alert(gettext("There are already 100 received messages.\nPlease delete some messages otherwise new messages will be lost."));
				}
				for(var j = 0; j < sms_inbox_list.length; j++){
					drawSMSInboxList(
						document.getElementById('sms_inbox_table'), 
						sms_inbox_list[j].new_sms,
						sms_inbox_list[j].name,
						sms_inbox_list[j].number,
						sms_inbox_list[j].time, 
						sms_inbox_list[j].content,
						j);
				}
			}else if(sms_inbox_list.length < temp_inbox_count){
				var index = parseInt(sms_inbox_list.length/10);
				SendJsonAction("get_sms_inbox_records", {"action":"get_sms_inbox_records","args":{"segment":index+1,"full_content":false}}, GetSMSInboxInfoCB, null);
			}
		}
	}else{
		$().toastmessage('showNoticeToast', gettext(msg.get_sms_inbox_records.errmsg));
	}
}

function LoadSMSInbox()
{
	if(temp_inbox_count > 0){
		SendJsonAction("get_sms_inbox_records", {"action":"get_sms_inbox_records","args":{"segment":1,"full_content":false}}, GetSMSInboxInfoCB, null);
	}else if(temp_inbox_count == 0){
		$("#short_inbox_total_num").text(0);
	}
}

function GetSMSOutboxInfoCB(msg)
{
	if(msg.get_sms_outbox_records.errno == 0){
		var outboxLength = msg.get_sms_outbox_records.records.length;
		if ($("#sms_outbox_table").length > 0){
			if (outboxLength > 0){
				for (var i = 0; i < outboxLength; i++){
					...
					for(var j=0;j<msg.get_sms_outbox_records.records[i].sendto.length;j++){
						if(msg.get_sms_outbox_records.records[i].sendto[j].name != "")
							temp_outbox_name = temp_outbox_name + msg.get_sms_outbox_records.records[i].sendto[j].name + "; ";
						else
							temp_outbox_name = temp_outbox_name + msg.get_sms_outbox_records.records[i].sendto[j].number + "; ";
					}
					
					temp_outbox.name = temp_outbox_name;
					temp_outbox.time = msg.get_sms_outbox_records.records[i].time;
					temp_outbox.content = msg.get_sms_outbox_records.records[i].content;
					temp_outbox.idx = msg.get_sms_outbox_records.records[i].idx;
				...
				if(sms_outbox_list.length == temp_outbox_count){
					$("#short_outbox_total_num").text(sms_outbox_list.length);
					for(var k = 0; k < sms_outbox_list.length; k++){
						var arr = sms_outbox_list[k].name.split(";");
						var temp_name = ""; 	   //display table
						for(var m=0;m<arr.length;m++){
							if(m==0){
								temp_name = temp_name + arr[m];
							}
						}
						if(arr.length>2){
							temp_name = temp_name + gettext(", and ")+(arr.length-2)+gettext("ppl.");
						}
						drawSMSOutboxList(
						document.getElementById('sms_outbox_table'), 
						temp_name,
						sms_outbox_list[k].time, 
						sms_outbox_list[k].content,
						k);
					}
				}else if(sms_outbox_list.length < temp_outbox_count){
					var index = parseInt(sms_outbox_list.length/10);
					SendJsonAction("get_sms_outbox_records", {"action":"get_sms_outbox_records","args":{"segment":index+1,"full_content":false}}, GetSMSOutboxInfoCB, null);
				}
			}
		}
	}else{
		$().toastmessage('showNoticeToast', gettext(msg.get_sms_outbox_records.errmsg));
	}
}

function LoadSMSOutbox()
{
	if(temp_outbox_count> 0){
		SendJsonAction("get_sms_outbox_records", {"action":"get_sms_outbox_records","args":{"segment":1,"full_content":false}}, GetSMSOutboxInfoCB, null);
	}else if(temp_outbox_count == 0){
		$("#short_outbox_total_num").text(0);
	}
}

function GetSMSDraftInfoCB(msg)
{
	if(msg.get_sms_draft_records.errno == 0){
		var draftLength = msg.get_sms_draft_records.records.length;
		if ($("#sms_draft_table").length > 0){
			if (draftLength > 0){
				for (var i = 0; i < draftLength; i++){
		 			var temp_draft = new Object();
					//var temp_name = ""; 	  //display table
					var temp_draft_name = ""; //display details
					for(var j=0;j<msg.get_sms_draft_records.records[i].sendto.length;j++) {			
						if(msg.get_sms_draft_records.records[i].sendto[j].name != "")			
							temp_draft_name = temp_draft_name + msg.get_sms_draft_records.records[i].sendto[j].name + "; ";
						else
							temp_draft_name = temp_draft_name + msg.get_sms_draft_records.records[i].sendto[j].number + "; ";
					}
					temp_draft.sendto = temp_draft_name;					
					temp_draft.time = msg.get_sms_draft_records.records[i].time;
					temp_draft.content = msg.get_sms_draft_records.records[i].content;
					temp_draft.idx = msg.get_sms_draft_records.records[i].idx;
				...
				if(sms_draft_list.length == temp_draft_count){
					$("#short_draft_total_num").text(sms_draft_list.length);	
					for(var k = 0; k < sms_draft_list.length; k++){
						var arr = sms_draft_list[k].sendto.split(";");
						var temp_name = ""; 	   //display table
						for(var m=0;m<arr.length;m++){
							if(m==0){
								temp_name = temp_name + arr[m];
						...
						drawSMSDraftList(
							...
							temp_name,
							sms_draft_list[k].time, 
							sms_draft_list[k].content,
							k);
					}
				}else if(sms_draft_list.length < temp_draft_count){
					var index = parseInt(sms_draft_list.length/10);
					SendJsonAction("get_sms_draft_records", {"action":"get_sms_draft_records","args":{"segment":index+1,"full_content":false}}, GetSMSDraftInfoCB, null);
				}
			}
		}
	}else{
		$().toastmessage('showNoticeToast', gettext(msg.get_sms_draft_records.errmsg));
	}
}

function LoadSMSDraft()
{
	if(temp_draft_count> 0){
		SendJsonAction("get_sms_draft_records", {"action":"get_sms_draft_records","args":{"segment":1,"full_content":false}}, GetSMSDraftInfoCB, null);
	}else if(temp_draft_count == 0){
		$("#short_draft_total_num").text(0);
	}
}

function GetSIMSMSInfoCB(msg)
{
	if(msg.get_sms_sim_records.errno == 0){
		var simLength = msg.get_sms_sim_records.records.length;
		if ($("#sim_sms_table").length > 0){
			if (simLength > 0) {
		 		for (var i = 0; i < simLength; i++) {
		 			var temp_sms = new Object();
					if (typeof(msg.get_sms_sim_records.records[i].sms_sent)=="undefined")
						temp_sms.sms_sent = false;
					else
						temp_sms.sms_sent = msg.get_sms_sim_records.records[i].sms_sent;
					temp_sms.new_sms = msg.get_sms_sim_records.records[i].new_sms;
					temp_sms.name = msg.get_sms_sim_records.records[i].name;
					temp_sms.number = msg.get_sms_sim_records.records[i].number;
					temp_sms.time = msg.get_sms_sim_records.records[i].time;
					temp_sms.content = msg.get_sms_sim_records.records[i].content;
					temp_sms.idx = msg.get_sms_sim_records.records[i].idx;
					sim_sms_list.push(temp_sms);
		 			
					temp_sms = null;
		 		}
				if(sim_sms_list.length == temp_sim_sms_count){
					$("#short_sms_total_num").text(sim_sms_list.length);
					for(var j = 0; j < sim_sms_list.length; j++){
						drawSIMSMSList(
							document.getElementById('sim_sms_table'), 
							sim_sms_list[j].new_sms,
							sim_sms_list[j].sms_sent,
							sim_sms_list[j].name,
							sim_sms_list[j].number,
							sim_sms_list[j].time, 
							sim_sms_list[j].content,
							j);

					}
				}else if(sim_sms_list.length < temp_sim_sms_count){
					var index = parseInt(sim_sms_list.length/10);
					SendJsonAction("get_sms_sim_records", {"action":"get_sms_sim_records","args":{"segment":index+1,"full_content":false}}, GetSIMSMSInfoCB, null);
				}
			}
		}
		
	}else{
		$().toastmessage('showNoticeToast', gettext(msg.get_sms_sim_records.errmsg));
	}
}

function LoadSIMSMS()
{
	if(temp_sim_sms_count > 0 ){
		SendJsonAction("get_sms_sim_records", {"action":"get_sms_sim_records","args":{"segment":1,"full_content":false}}, GetSIMSMSInfoCB, null);
	}else if(temp_sim_sms_count == 0){
		$("#short_sms_total_num").text(0);
	}
}
...
function GetInboxInfo(inbox_info)
{
	...
	else
	{
		...
		if (inbox_info.records.length >= 100)
		{
			alert(gettext("There are already 100 received messages.\nPlease delete some messages otherwise new messages will be lost."));
		}
		...
		if ($("#sms_outbox_table").length > 0) 
		{
		 	...
		 	if (outbox_info.records.length > 0) 
			{
		 		for (var i = 0; i < outbox_info.records.length; i++)
		 		{
					...
					{
						temp_name = temp_name + gettext(", and ")+(outbox_info.records[i].sendto.length-1)+gettext("ppl.");
					}
		...
		if ($("#sms_draft_table").length > 0) 
		{
		 	...
		 	if (sms_draft_info.records.length > 0) 
			{
		 		for (var i = 0; i < sms_draft_info.records.length; i++)
		 		{
					...
					{
						temp_name = temp_name + gettext(", and ")+(sms_draft_info.records[i].sendto.length-1)+gettext("ppl.");
					}										
...
function GetSMSCenterNumber(center_number_info)
{
	...
	{
		$("#sms_center_number").val(center_number_info.sc_addr);
	}
}
	...
	$.ajax({
		...
		{						
			if (xhr.status)
			{
				...
				{
					if (item == "get_wwan_pin_status")
					{
						GetWWANPinStatus(msg.get_wwan_pin_status);
					}
					else if(item == "get_sms_ind")
					{
						GetSMSInd(msg.get_sms_ind);
					}				
					else if(item == "get_sms_info")
					{
						sms_flag = type;
						GetSMSInfo(msg.get_sms_info);
					}
					else if(item == "get_sms_inbox_records")
					{								
						if (!type)
							GetInboxInfo(msg.get_sms_inbox_records);
						else
							inbox_popup_one(msg.get_sms_inbox_records, start);					
					}	
					else if (item == "get_sms_outbox_records")
					{	
						if (!type)
							GetOutboxInfo(msg.get_sms_outbox_records);
						else
							outbox_popup_one(msg.get_sms_outbox_records, start);
					}
					else if(item == "get_sms_sim_records")
					{
						if (!type)
							GetSIMSMSInfo(msg.get_sms_sim_records);
						else
							sim_sms_popup_one(msg.get_sms_sim_records, start);						
					}	
					else if(item == "get_sms_draft_records")
					{
						if (!type)
							GetDraftInfo(msg.get_sms_draft_records);
						else
						{
							if (edit)
								draft_edit_popup_one(msg.get_sms_draft_records, start);
							else if (send)
								draft_send_record(msg.get_sms_draft_records, start);
							else
								draft_popup_one(msg.get_sms_draft_records, start);
						}
					}
					else if(item == "get_sms_center_number")
					{					
						GetSMSCenterNumber(msg.get_sms_center_number);
					}
				}
				unblockUI();
			} 
		...
		success: function(msg, textStatus, xhr) {
			if (xhr.status) {					
				...
				else
				{
					...
					if (item=="set_sms_send" && typeof(msg.set_sms_send)!="undefined" && msg.set_sms_send.errno < 0)
					{
						ok = false;
						//alert(msg.set_sms_send.errmsg);
					}
					else if (item=="set_sms_inbox_deleted" && typeof(msg.set_sms_inbox_deleted)!="undefined" && msg.set_sms_inbox_deleted.errno < 0)
					{
						ok = false;
						alert(gettext(msg.set_sms_inbox_deleted.errmsg));
					}
					else if (item=="set_sms_inbox_read" && typeof(msg.set_sms_inbox_read)!="undefined" && msg.set_sms_inbox_read.errno < 0)
					{
						ok = false;
						alert(gettext(msg.set_sms_inbox_read.errmsg));
					}
					else if (item=="set_sms_save_draft" && typeof(msg.set_sms_save_draft)!="undefined" && msg.set_sms_save_draft.errno < 0)
					{
						ok = false;
						alert(gettext(msg.set_sms_save_draft.errmsg));
					}
					else if (item=="set_sms_outbox_deleted" && typeof(msg.set_sms_outbox_deleted)!="undefined" && msg.set_sms_outbox_deleted.errno < 0)
					{
						ok = false;
						alert(gettext(msg.set_sms_outbox_deleted.errmsg));
					}
					else if (item=="set_sms_sim_deleted" && typeof(msg.set_sms_sim_deleted)!="undefined" && msg.set_sms_sim_deleted.errno < 0)
					{
						ok = false;
						alert(gettext(msg.set_sms_sim_deleted.errmsg));
					}
					else if (item=="set_sms_sim_read" && typeof(msg.set_sms_sim_read)!="undefined" && msg.set_sms_sim_read.errno < 0)
					{
						ok = false;
						alert(gettext(msg.set_sms_sim_read.errmsg));
					}
					else if (item=="set_sms_draft_deleted" && typeof(msg.set_sms_draft_deleted)!="undefined" && msg.set_sms_draft_deleted.errno < 0)
					{
						ok = false;
						alert(gettext(msg.set_sms_draft_deleted.errmsg));
					}				
					else if (item=="set_sms_center_number" && typeof(msg.set_sms_center_number)!="undefined" &&  msg.set_sms_center_number.errno < 0)
					{
						ok = false;
						alert(gettext(msg.set_sms_center_number.errmsg));
					}
				}
...
$(document).ready(function()
{
	...
	*/
	...
	if ($("#get_new_sms_info").length > 0)
	{
		$('#new_sms_send_to').val("");
		$('#new_sms_content').val("");		
		$(function(){LoadSMSInfo('get_wwan_pin_status', false);})
		$(function(){LoadSMSInfo('get_sms_ind', false);})
	...
	{
		$(function(){LoadSMSInfo('get_sms_ind', false);})
		$(function(){LoadSMSInfo('get_sms_info',"inbox");})	
		//$(function(){LoadSMSInfo('get_sms_inbox_records', false);})
	}	
	if ($("#get_sms_outbox_info").length > 0) 
	{
		$(function(){LoadSMSInfo('get_sms_ind', false);})
		$(function(){LoadSMSInfo('get_sms_info',"outbox");})	
	...
	if ($("#get_sim_sms_info").length > 0)
	{
		$(function(){LoadSMSInfo('get_wwan_pin_status', false);})
		$(function(){LoadSMSInfo('get_sms_ind', false);})
		$(function(){LoadSMSInfo('get_sms_info',"sim_sms");})
	...
	if ($("#get_sms_draft_info").length > 0)
	{
		$(function(){LoadSMSInfo('get_wwan_pin_status', false);})
		$(function(){LoadSMSInfo('get_sms_ind', false);})
		$(function(){LoadSMSInfo('get_sms_info',"draft");})
	...
	$('#save_new_sms').live('click', function(){
		if (temp_draft_count >= 100)
		{
			alert(gettext("There are already 100 draft messages."));
			return false;
		}
		if (submit_new_sms_check())		
		{

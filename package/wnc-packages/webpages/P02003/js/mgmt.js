...
function accessLevelCheck ()
    ...
    if (parseInt (accessLevel, 10) != 0)
        {
        lblWarning.innerHTML = gettext("Administrator privilages required");
        return false;
        }
    return true;
    }
		...
		if(exten == "none") {
			...
			if (ext.length > 4) {	
				...
				if (exten == "img"){	//firmware image
					...
					alert(i18n_selecta+"bin"+i18n_file);
				}
				return false;
			} else if(ext != exten) {
				//alerT("ext="+ext+"\n exten="+exten);
				...
				if (exten == "img"){
					...
					if (ext != "bin") {
				  		if(ext.length == 0)
							alert(i18n_dontSelect+"bin"+i18n_fileInstead);
						else
	    					alert(i18n_youSelect+ext+i18n_pleaseSelect+"bin"+i18n_fileInstead);
					  	return false;
					}  
				} 	
			} 

...
{
	var i18n_enterSchName = gettext("Please enter a valid Schedule Name");
	var i18n_invalidHoursStart = gettext("Invalid start time for schedule. Enter valid hour between 1 and 12");
	var i18n_invalidStartTime = gettext("Invalid start time for schedule.");
	var i18n_hours = gettext("hours");
	var i18n_invalidMinsStart = gettext("Invalid start time for schedule. Enter valid minutes between 0 and 59");
	var i18n_mins = gettext("minutes");
	var i18n_invalidHoursEnd = gettext("Invalid end time for schedule. Enter valid hour between 1 and 12");
	var i18n_invalidEndTime = gettext("Invalid end time for schedule.");
	var i18n_invalidMinsEnd = gettext("Invalid end time for schedule. Enter valid minutes between 0 and 59");
...
{
	var i18n_invalidHours112 = gettext("Invalid Hours: Please enter a value between 1 - 12");
	var i18n_invalidMins059 = gettext("Invalid Minutes: Please enter a value between 0 - 59");
...
function timeValidate()
{
    ...
    var i18n_endLessStart = gettext("End hour cannot be less than the start hour");
	var i18n_endHrsNextDay = gettext("End Time hours cannot extend to next day");
	...
	if (isNaN(hr1))
	{
		alert(gettext("Invalid Start time. Please enter a value between 1 - 12 hours."));
		return false;
	}
	else if (isNaN(hr2))
	{	
		alert(gettext("Invalid End time. Please enter a value between 1 - 12 hours."));
		return false;
	}
	else if (isNaN(min1))
	{
		alert(gettext("Invalid Start time. Please enter a value between 0 - 59 minutes."));		
		return false;
	}
	else if (isNaN(min2))
	{
		alert(gettext("Invalid End time. Please enter a value between 0 - 59 minutes."));
		return false;		
	}	
    ...
    {		
		if(hr1==hr2)
		{
			if(min1 >= min2)
			{
				alert(gettext("Please check minutes"));
				return false;
			}
		}
		else if(hr1>hr2)
...
{
	var i18n_selDaysOnSch = gettext("Please select day(s) on which this schedule should apply.");
...
function chkSelOptTmDay ()
{
  ...
  { //5AM-12AM
	...
	document.getElementById("selEndMeridian").value="AM";			  
  }

...
function deviceReboot()
{
	...
	if (Objdevice)
	{
		...
		document.RESETFORM.action="/goform/sysReboot?<%getUniName();%>=<%getUniId();%>" ;
...
function factory_reset()
{
	...
	if (factrst)
	{
		...
		document.RESETFORM.action="/goform/factoryReset?<%getUniName();%>=<%getUniId();%>";
		document.RESETFORM.submit();
	}
	else
	{
...
<div class="contentBg">
	...
	<div class="secBg">
		...
		<div class="secInfo">
			<br><script>document.write(gettext("After completion of the process the login page of the browser will be shown."));</script>
			<br>
		</div>

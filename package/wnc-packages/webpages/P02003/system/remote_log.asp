...
function fnApply()
{
	...
	txtFieldIdArr[0] = "lan_hostname," + gettext("Invalid Host Name Format");
	txtFieldIdArr[1] = "lan_ip," + gettext("Invalid IP address Format");
	txtFieldIdArr[2] = "lan_submask," + gettext("Invalid Subnet Mask Format");
	...
	{
		if (ipv4Validate("remote_ip", gettext("IP"), false, true, "", "", true) == false) 
		{
			return false;
		}
	}
...
<div class="contentBg">
	...
	<div class="secBg">
		...
		<form name="SYSLOG" action="/goform/setRemoteLog?<%getUniName();%>=<%getUniId();%>" method="post">
  ...
  <div class="secH2"><script>document.write(gettext("Download tar log files"));</script></div>
		...
		<br />
		<form name="syslog_setting" action="/goform/setSysLog?<%getUniName();%>=<%getUniId();%>" method="post">

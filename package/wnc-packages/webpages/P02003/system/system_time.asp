...
function GetTextCurrentTime()
{
	var curTimeInfo = "<%getCurrTime();%>";
	var curTimeInfoArr = curTimeInfo.split(", ");
	var curMonthInfoArr = curTimeInfoArr[1].split(" ");
	var curTime = gettext(curTimeInfoArr[0]) + ", " + gettext(curMonthInfoArr[0]) + " " + curMonthInfoArr[1] + ", " + curTimeInfoArr[2] + ", " + curTimeInfoArr[3];
...
function timeZoneValidate()
{
	if(!CheckLoginInfo())
		return false;
	var i18n_validTimeServer = gettext("Please enter a valid time-server name or IP address for server");
    ...
    var i18n_ipaddrDomain = gettext("Invalid NTP Server.");//"Invalid IP Address/Domain name";
...
<div class="contentBg">
	...
	<div class="secBg">
		...
		<form name="ntp_setting" action="/goform/setNTPConfig?<%getUniName();%>=<%getUniId();%>" method="post">

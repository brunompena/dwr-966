...
function checkDesLength()
{
	var user_description = document.getElementById ("user_description_display").value;
	if(user_description.length>100){
		document.getElementById ("user_description_display").value = user_description.substring(0,100);
		alert(gettext("The 101th character cannot be input to the textbox."));
		return false;
	}
}
...
function user_validate()
{
	...
	var descript_rule = /^[a-zA-Z0-9\_\-\.\,\ ]+$/;

	if(temp_state == "edit" && temp_index == 1)
	{
	...
	if (user_description != "") {
		if(user_description.length>100){
			...
			if(!descript_rule.test(user_description)){
			alert(gettext("Descript: Accept 0-9,a-z,A-Z,' ',_,-,. and , or not accept Chinese format."));
			document.getElementById("user_description_display").focus();
			return false;
			}
		}
	}	
...
<br></div>
		<form name="user_account_edit" method="post" action="/goform/setUserAccount?<%getUniName();%>=<%getUniId();%>">
...
<!--				onkeydown="if (event.keyCode == 9) {return fieldLengthCheck ('txtpwd', 5, 32, 'Password lenght should be in between 5 - 32 characters');}"-->
			...
			<tr>
				...
				<td><textarea rows="2" id="user_description_display" class="configF1" name="description" onkeyup="checkDesLength();"></textarea></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
				 	...
				 	<span class="spanText"><script>document.write(gettext("seconds"));</script></span>
				</td>
			</tr>
		</table>
		<div class="submitBg">

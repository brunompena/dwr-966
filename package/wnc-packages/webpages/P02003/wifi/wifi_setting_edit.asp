...
function checkWIFIEdit()
{
   ...
   if(!code_rule.test(ssid1)){
	...
	if ((wlan == "1")||(wlan == "2")){  
		...
		if (ap1_sec_type == "WEP"){
			...
			if(wep_key1.length == 5){
				if(!wep_rule1.test(wep_key1)){
					alert(gettext("WEP Password: 5 charts accept only 0-9,a-z,A-Z."));
					document.getElementById("wepkey01").focus();
					return false;
				}
			}else if(wep_key1.length == 13){
				if(!wep_rule2.test(wep_key1)){
					alert(gettext("WEP Password: 13 charts accept only 0-9,a-z,A-Z."));
					document.getElementById("wepkey01").focus();
					return false;
				}
			}else if(wep_key1.length == 10){
				if(!wep_rule3.test(wep_key1)){
					alert(gettext("WEP Password: 10 charts accept only 0-9,a-f,A-F."));
					document.getElementById("wepkey01").focus();
					return false;
				}
			}else if(wep_key1.length == 26){
				if(!wep_rule4.test(wep_key1)){
					alert(gettext("WEP Password: 26 charts accept only 0-9,a-f,A-F."));
					document.getElementById("wepkey01").focus();
					return false;
				}
			}else{
				alert(gettext("WEP Password: ASCII: 5 or 13 0-9, a-z, A-Z charts. Hex: 10 or 26 0-9, a-f, A-F charts."));
				document.getElementById("wepkey01").focus();
				return false;
			}
		...
		if (ap1_sec_type == "WPA" || ap1_sec_type == "WPA2" || ap1_sec_type == "WPAWPA2") {
		     if(pwd1.length < 8 || pwd1.length > 64){
				alert(gettext("The WPA2/WPA+WPA2 input should be Hex 64 charts or Ascii 8~63 charts"));
				document.getElementById("pskascii1").focus();
				return false;
		     }else if(pwd1.length > 7 && pwd1.length < 64){
				if(!pwd_rule1.test(pwd1)){
					alert(gettext("WPA2/WPA+WPA2 Password: Accept only 0-9,a-z,A-Z."));
					document.getElementById("pskascii1").focus();
					return false;
				}
		     }else if(pwd1.length > 63){
				if(!pwd_rule2.test(pwd1)){
					alert(gettext("WPA2/WPA+WPA2 Password: Accept only 0-9,a-f,A-F."));
					document.getElementById("pskascii1").focus();
					return false;
				}
		     }
...
<!-- Main Menu and Submenu End -->
 ...
 <div class="contentBg">
    ...
    <div class="secInfo"><br><script>document.write(gettext("In this section you can add/edit a wireless configuration to wireless functionality."));</script>
                         <br>&nbsp;&nbsp;<a href="wifi_settings.asp" class="secLable1">&#187; <script>document.write(gettext("Back to Wi-Fi Settings page"));</script></a>
    </div>
    <form name="wifi_setting_edit" action="/goform/setWiFiEdit?<%getUniName();%>=<%getUniId();%>" method="post">
    ...
    <table class="configTbl" cellspacing="0" >
      ...
      <tr id="security1" style="display:none;">
    	...
    	<td><select name="enc_idx1" id="enc_idx1" onChange="encgroup();" class="configF1">
			...
			<option value="WPAWPA2">WPA+WPA2</option>  
        </select></td>
  	  </tr>
    ...
    <div class="secH2"><script>document.write(gettext("Advanced Configuration"));</script></div>	
    <br />
    <table class="configTbl" cellspacing="0" >
      <tr>

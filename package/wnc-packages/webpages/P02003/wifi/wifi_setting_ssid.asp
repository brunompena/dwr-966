...
function wifi_op_check(state)
{
    ...
    if (state == "add") {
		if (objArr.length-1 == 5){
			alert(gettext("The schedule max count is 5."));
			ret=false;
		}	
	  ...
	  if (count==0)	{
		...
		ret=false;
	  	...
	  	if (statusCount > 0){
		  alert(gettext("Schedule rule selected to delete is in use."));
		  ret=false;	
	  	}else{	
...
<!-- Main Menu and Submenu End -->
 ...
 <div class="contentBg">
   ...
   <form name="wifi_setting_ssid" method="post" action="/goform/setWifiSSID?<%getUniName();%>=<%getUniId();%>" > 

...
function checkWIFIssidEdit()
{
   ...
   if (edit_flag==0){
     for(row = 0; row < length; row += 1){
	    ...
	    alert(gettext("The schedule name is duplicated, can't be used."));	
	    return false;
  	  }  
     }
   }   
...
/*
   if( meridian1 == meridian2 ){
     if(hr1 == hr2){
       if(min1 >= min2) {
   	     alert(gettext("Start Minute should be less than Stop Minute."));
     	 return false;
       }else{
         if (min < 5){
		   alert(gettext("Stop Time should be more than Start Time at least 5 minutes."));
		   document.getElementById('stopMinute').focus();
     	   return false;	
	   	 }
       }
       ...
       if (hr1 != 12) {
         alert(gettext("Start Hour should be less than Stop Hour."));
     	 return false;
       }
     } else {  //hr1 < hr2
	   ...
	   if (Time < 5){
    	...
    	alert(gettext("The Start Meridian should be less than Stop Meridian."));
    	return false;
   }
*/
...
<!-- Main Menu and Submenu End -->
 ...
 <div class="contentBg">
    ...
    <form method="post" action="/goform/setWiFiSSIDEdit?<%getUniName();%>=<%getUniId();%>">
	...
	<div>
	  ...
	  <input type="reset" id="edit_reset" value="Reset" class="submit" onmouseover="this.className = 'submitHover'" onmouseout="this.className = 'submit'" onclick="doRedirect();"/>
	</div>
	<!--Button End-->
	</form>
   </div>

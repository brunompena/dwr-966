...
function pressButton(state)
{
    ...
    if ((state == "edit")) {
	  ...
	  if (count > 1){
	  	alert(gettext("Please select one row to set schedule."));
		ret=false;
	  }else if (count==0)	{
        alert(gettext("Please select a row from the list to set schedule."));
		ret=false;
	  }	else {
	  ...
	  if (count > 1){
	  	alert(gettext("Please select one row to set ACL."));
	  	ret=false;
	  }else if (count==0)	{
        alert(gettext("Please select a row from the list to set acl."));
		ret=false;
	  }	else {
...
<!-- Main Menu and Submenu End -->
 ...
 <div class="contentBg" >
   ...
   <div class="secInfo"><br><script>document.write(gettext("The status of the available Access Points are displayed here."));</script><br></div>
    <form name="wifi_setting" method="post" action="/goform/setWiFiAP?<%getUniName();%>=<%getUniId();%>" > 
    ...
    <!--AP List-->
	<table cellspacing="0" id="tblWiFiSetting" width="720px" style="table-layout: fixed;" class="specTbl">
      <tr>
		...
		<td class="tdH" width="4%"><input type="hidden" name="imgSelectAllChk" id="imgSelectAll" title="Select All" onclick="secChkBoxSelectOrUnselectAll ('tblWiFiSetting', this)"></td-->
      	<td class="tdH" width="10%"><script>document.write(gettext("Status"));</script></td>
      	<td class="tdH" width="22%"><script>document.write(gettext("SSID"));</script></td>
      	<td id="wifi_frequence" class="tdH" width="11%"><script>document.write(gettext("Frequence"));</script></td>
      	<td id="wifi_broadcast" class="tdH" width="11%"><script>document.write(gettext("Broadcast"));</script></td>
      	<td id="wifi_security" class="tdH" width="12%"><script>document.write(gettext("Security"));</script></td>
      	<td class="tdH" width="10%"><script>document.write(gettext("Active"));</script><br><script>document.write(gettext("Time"));</script></td>
      	<td class="tdH" width="10%"><script>document.write(gettext("Start"));</script><br><script>document.write(gettext("Time"));</script></td>
      	<td class="tdH" width="11%"><script>document.write(gettext("Stop"));</script><br><script>document.write(gettext("Time"));</script></td>
      </tr>
      <%getWiFiAP();%>
    </table>  
  	<!--Button Start-->
...
<script type="text/javascript">
	...
	document.getElementById('acl_wifi').value=gettext("MAC Filtering");
	if (document.getElementById('select_language').value != "en") {
		document.getElementById('ssid_wifi').style.fontSize = "9px";
		document.getElementById('wifi_frequence').title=gettext("Frequence");
		document.getElementById('wifi_broadcast').title=gettext("Broadcast");
		document.getElementById('wifi_security').title=gettext("Security");
	}
</script>
</body>
</html>

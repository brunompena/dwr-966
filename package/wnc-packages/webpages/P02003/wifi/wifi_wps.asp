	...
	var WPSinfoArr3 = WPSinfo3.split("#");
 	...
 	if(Ele.id == "PBC1") {	
      if (WPSinfoArr[12] == "1") {            //SSID1
    	...
    	document.wps_pbc1.action="/goform/setWPSPBC1?<%getUniName();%>=<%getUniId();%>";
        ...
        alert(gettext("It can not work when WPS of SSID1 or SSID3 is disabled."));
    	return false;
      }
   	} else if(Ele.id == "PBC2") {
	  if (WPSinfoArr2[12] == "1"){    		//SSID2
		...
		document.wps_pbc2.action="/goform/setWPSPBC2?<%getUniName();%>=<%getUniId();%>";
...
function enterPinValidate(Ele)
{
	...
	if(Ele.id == "wps_pin1")
	{
		...
		if (WPSinfoArr[12] == "1") {          //SSID1
    		...
    		document.wps_pin1.action="/goform/setWPSPIN1?<%getUniName();%>=<%getUniId();%>";
	...
	else if(Ele.id == "wps_pin2")
	{
		...
		document.wps_pin2.action="/goform/setWPSPIN2?<%getUniName();%>=<%getUniId();%>";
...
{   
    if (WPSinfoArr3[12] == "0"){
      ...
      if (state == true){	
	  	alert(gettext("The SSID3 is enabled. The SSID1 cannot be set to be enabled."));
		document.getElementById("ap_en1").checked = false;
		return false;
      }else{
      	document.getElementById("ap_en1").checked = state;  
...
function WPS3Disable(state)
{
    if (WPSinfoArr[12] == "0"){
      ...
      if (state == true){	
	  	alert(gettext("The SSID1 is enabled. The SSID3 cannot be set to be enabled."));
		document.getElementById("ap_en3").checked = false;
		return false;
      }else{
      	document.getElementById("ap_en3").checked = state;  
...
function submitActionWPS1(Ele)
{
	...
	if (sSSID == 0){
	  if(WPSinfoArr[3] == "WEP")
	  {
		alert(gettext("WPS1 function can not work when AP1 Security Type is WEP."));
		doRedirect();
		return false;
  	  } 	
	  document.getElementById('ssid_dummy').value = "0";	
      ...
      if(WPSinfoArr3[3] == "WEP")
	  {
		alert(gettext("WPS3 function can not work when AP3 Security Type is WEP."));
		return false;
  	  } 
	  document.getElementById('ssid_dummy').value = "1";	
  	...
  	document.AP_ENABLE.action="/goform/setWPS1Enable?<%getUniName();%>=<%getUniId();%>" ;
...
function submitActionWPS2(Ele)
{
	...
	else
	{
  	...
  	document.AP2_ENABLE.action="/goform/setWPS2Enable?<%getUniName();%>=<%getUniId();%>";
...
<div class="contentBg">
  ...
  <div class="secInfo"><br><script>document.write(gettext("WPS is a standard for easy and secure setup of a wireless connection. In this section you can enable WPS for a client connection using OPEN/WPA2 security."));</script><br></div>
  ...
  <div>
	...
	<input type="submit" id="reset_wps1" value="Cancel" class="tblbtn" title="Cancel" name="" onmouseover="this.className = 'tblbtnHover'" onmouseout="this.className = 'tblbtn'" onclick="doRedirect();">
  </div>
  </form>
  </div>
  ...
  <div>
	...
	<input type="submit" id="reset_wps2" value="Cancel" class="tblbtn" title="Cancel" name="" onmouseover="this.className = 'tblbtnHover'" onmouseout="this.className = 'tblbtn'" onclick="doRedirect();">
  </div>
  </form>
  </div>
...
<script type="text/javascript">
  ...
  WPSinfo3 = <%getWifi(2);%>;
  WPSinfoArr3 = WPSinfo3.split("#");
  var SSID1 = WPSinfoArr[12];
  var SSID3 = WPSinfoArr3[12];
  if (WPSinfoArr[12] == "1"){
	  document.getElementById("ssidOption").selectedIndex = 0;
  }
  if (WPSinfoArr3[12] == "1"){
	  document.getElementById("ssidOption").selectedIndex = 1;
  }
  document.getElementById('apply_wps1').value=gettext("Apply");
  document.getElementById('reset_wps1').value=gettext("Cancel");
  ...
  document.getElementById('reset_wps2').value=gettext("Cancel");

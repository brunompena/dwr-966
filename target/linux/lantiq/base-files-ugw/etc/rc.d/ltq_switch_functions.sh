...
################################################
  ...
  "0"|"3")
    ...
    if [ "$CONFIG_IFX_CONFIG_CPU" != "AMAZON_S" ]; then
      ...
      #if [ "A$CONFIG_FEATURE_DSL_BONDING_SUPPORT" = "A1" -a "$CONFIG_WAN" = "3" ]; then
      # the packet path for PTM packets in Bonding Model is the same for ADSL and VDSL.

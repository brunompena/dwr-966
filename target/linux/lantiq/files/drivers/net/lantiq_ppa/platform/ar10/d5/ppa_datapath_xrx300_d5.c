/******************************************************************************
**
** FILE NAME    : ppa_datapath_ar10_d5.c
** PROJECT      : UEIP
...
//#include "ppa_datapath_fw_ar10_d5.h"
//#include "ppa_hal_ar10_d5.h"
#include "ppa_datapath_fw_xrx300_d5.h"
#include "ppa_hal_xrx300_d5.h"
...
#define VRX218_DMA_RX_CHAN_NO                   3
...
#define DMA_RX_CH3_DESC_PPE_BASE                0x37E0
...
#define DMA_RX_CH3_DESC_BASE                    ((volatile struct rx_descriptor *)SB_BUFFER(DMA_RX_CH3_DESC_PPE_BASE))                /*         32 each queue    */
...
static int proc_write_vrx218_swap_seq(struct file *, const char __user *, size_t, loff_t *);
...
static INLINE int alloc_dma(void)
{
    ...
    pre_alloc_desc_total_num = VRX218_SYN_DESC_NUM + DMA_RX_CH3_DESC_NUM;
    if(!dma_loopback){
    ...
    //dma rx channel 3
    p = (volatile u32 *)DMA_RX_CH3_DESC_BASE;
...
static INLINE void init_vrx218_fpg_swap_cfg(void)
{
    ...
    swap_cfg->cdma_des_num          = DMA_RX_CH3_DESC_NUM;
    swap_cfg->vrx218_syncdes_num    = VRX218_SYN_DESC_NUM;
    swap_cfg->cdma_des_base         = DMA_RX_CH3_DESC_PPE_BASE;
...
static struct file_operations g_proc_file_vrx218_swap_seq_fops = {
    ...
    .read       = seq_read,
    .write      = proc_write_vrx218_swap_seq,
    .llseek     = seq_lseek,
    .release    = single_release,
};
...
static int proc_read_qos(struct seq_file *seq, void *v)
{
    ...
    struct clk * clk ;
    clk = clk_get_ppe();
    seq_printf(seq, "  PPE clk=%u MHz, basic tick=%u\n", (unsigned int) (clk_get_rate(clk) / 1000000), TX_QOS_CFG->time_tick / (unsigned int) (clk_get_rate(clk) / 1000000));
#endif
...
static int proc_read_vrx218_swap(struct seq_file *seq, void *v)
{
    ...
    seq_printf(seq, "VRX218 SWAP_EN:@0x37D0:\n");
    ...
    seq_printf(seq, "vrx218_en:          %s\n",  (unsigned int) gencfg3->vrx218_en ? "Yes" : "No");
    seq_printf(seq, "vrx218 Chan[%d]     %s\n",  VRX218_DMA_RX_CHAN_NO, (unsigned int) swap_cfg->swap_en ? "ON" : "OFF");
    

    return 0;
}

static ssize_t proc_write_vrx218_swap_seq(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char local_buf[32];
    int len;
    char *p1, *p2;
    ...
    printk("echo <Enable | Disable>  > /proc/eth/vrx218_swap\n");
    return count;
...
#endif //CONFIG_LANTIQ_ETH_FRAMEWORK

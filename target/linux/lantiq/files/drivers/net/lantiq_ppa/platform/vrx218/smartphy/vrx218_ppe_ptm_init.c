...
{
    struct sk_buff *skb = alloc_skb_tx(1600);
...
void vrx218_pdma_init(unsigned long base, int is_bonding)
{
    ...
    *SOC_ACCESS_VRX218_ADDR(VRX_PPE_PDMA_RX_MAX_LEN_REG, base)       = is_bonding ? 0x02040204 : 0x02040604;
...
void vrx218_gen_cfg_init(unsigned long base, int is_bonding, unsigned int vrx218_id, unsigned long peer_base)
{
...
void vrx218_ds_soc_des_init(unsigned long base, int is_bonding, unsigned int vrx218_id)
{
    ...
    if (is_bonding == 0) {
        ...
        rx_descriptor.sop = 1;
        rx_descriptor.eop = 1;
        rx_descriptor.data_len = 1568;

        //Initialize DS Descriptors
        for (i=0; i<SOC_DS_DES_NUM; i++) {
            rx_descriptor.data_ptr = (unsigned int)alloc_dataptr_fragment(1600);
        ...
        rx_descriptor.data_len = 1568; //used to be 544 for bonding.  now we increase the size in order to support L2 truncking mode

        //Initialize DS Frag Des List Descriptors
        for (i=0; i<32; i++) {
            rx_descriptor.data_ptr = (unsigned int)alloc_dataptr_fragment(1600);//used to 560
        ...
        rx_descriptor.own = 1;
        rx_descriptor.byte_off = 2;
        rx_descriptor.data_len = 1568; //used to be 544 for bonding
        for (i=0; i<32; i++) {
            rx_descriptor.data_ptr = (unsigned int)alloc_dataptr_fragment(1600); //used to be 560
            dst_addr = (void *)SOC_ACCESS_VRX218_SB((__DS_FRAGQ_DES_LIST_BASE + (48 * 2) + (i * 2)), base); //ds_frag_des_list2 0x2730
            dword_mem_write(dst_addr, &rx_descriptor, sizeof(rx_descriptor));
        }
...
void vrx218_edma_copy_ch_init(unsigned long base, int is_bonding, unsigned int vrx218_id, int cdma_write_data_en)
{
    ...
    unsigned int *dst_addr, i, us_des_alloc[] = {15, 4, 0, 0};
...
void vrx218_local_des_init(unsigned long base)
{
    unsigned int i, *dst_addr;
    tx_descriptor_t tx_descriptor;
    rx_descriptor_t rx_descriptor;
    ...
    for (i=0; i<__US_TC_LOCAL_Q_DES_LIST_NUM; i++) {
        tx_descriptor.data_ptr = (PDBRAM_TX_PKT_BUFFER_BASE + (i * 1600));
    ...
    for (i=0; i<__DS_TC_LOCAL_Q_DES_LIST_NUM; i++) {
        rx_descriptor.data_ptr = (PDBRAM_RX_PKT_BUFFER_BASE + (i * 1600));
...
void __init vrx218_ppe_ptm_init(const ifx_pcie_ep_dev_t *dev, const ifx_pcie_ep_dev_t *peer_dev, int is_bonding, unsigned int vrx218_id, unsigned long sync_buf, int lle_in_sb, int cdma_write_data_en)
{
    ...
    vrx218_gen_cfg_init(base, is_bonding, vrx218_id, peer_base);
    ...
    vrx218_ds_soc_des_init(base, is_bonding, vrx218_id);

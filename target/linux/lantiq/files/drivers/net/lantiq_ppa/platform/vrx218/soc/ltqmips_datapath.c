...
//#include <ltq_dma.h>
//#include <ltq_clk.h>
//#include <asm/mach-ltqcpe/ltq_switch_api/ifx_ethsw_kernel_api.h>
//#include <asm/mach-ltqcpe/ltq_switch_api/ifx_ethsw_api.h>
//#include <switch_api/ifx_ethsw_kernel_api.h>
//#include <switch_api/ifx_ethsw_api.h>
#include <linux/version.h>
 ...
 *  GPIO Related
 */
//#define PPA_GPIO_OP(op, pin)                op((pin), IFX_GPIO_MODULE_PPA)
//#define IFX_SWITCH_PIN_RESERVE(pin)             PPA_GPIO_OP(ifx_gpio_pin_reserve, pin)
//#define IFX_SWITCH_DIR_OUT(pin)                 PPA_GPIO_OP(ifx_gpio_dir_out_set, pin)
//#define IFX_SWITCH_DIR_IN(pin)                  PPA_GPIO_OP(ifx_gpio_dir_in_set, pin)
//#define IFX_SWITCH_OUTPUT_SET(pin)              PPA_GPIO_OP(ifx_gpio_output_set, pin)
//#define IFX_SWITCH_OUTPUT_CLR(pin)              PPA_GPIO_OP(ifx_gpio_output_clear, pin)
//#define IFX_SWITCH_ALTSEL0_SET(pin)             PPA_GPIO_OP(ifx_gpio_altsel0_set, pin)
//#define IFX_SWITCH_ALTSEL0_CLR(pin)             PPA_GPIO_OP(ifx_gpio_altsel0_clear, pin)
//#define IFX_SWITCH_OD_SET(pin)                  PPA_GPIO_OP(ifx_gpio_open_drain_set, pin)
//#define IFX_SWITCH_OD_CLR(pin)                  PPA_GPIO_OP(ifx_gpio_open_drain_clear, pin)
//#define IFX_SWITCH_ALTSEL1_SET(pin)             PPA_GPIO_OP(ifx_gpio_altsel1_set, pin)
//#define IFX_SWITCH_ALTSEL1_CLR(pin)             PPA_GPIO_OP(ifx_gpio_altsel1_clear, pin)
//#define IFX_SWITCH_PUDSEL_SET(pin)              PPA_GPIO_OP(ifx_gpio_pudsel_set, pin)
//#define IFX_SWITCH_PUDEN_SET(pin)               PPA_GPIO_OP(ifx_gpio_puden_set, pin)
...
// This I will check in the revision 1348 ppa_drv repo
u32 ppa_get_pp32_clock(void);
...
static ssize_t proc_write_mib(struct file *, const char __user *, size_t, loff_t *);
static void clear_mib_counter(unsigned int );
#if defined(DEBUG_FIRMWARE_TABLES_PROC) && DEBUG_FIRMWARE_TABLES_PROC

  static ssize_t proc_write_route(struct file *, const char __user *, size_t, loff_t *);
  ...
  static ssize_t proc_write_genconf(struct file *, const char __user *, size_t, loff_t *);
  ...
  static ssize_t proc_write_hit(struct file *, const char __user *, size_t, loff_t *);
...
static ssize_t proc_write_burstlen(struct file *, const char __user *, size_t, loff_t *);
#if defined(DEBUG_FW_PROC) && DEBUG_FW_PROC
  static int proc_read_fw(struct seq_file *, void *);
  static ssize_t proc_write_fw(struct file *, const char __user *, size_t, loff_t *);
...
static ssize_t proc_write_directforwarding(struct file *, const char __user *, size_t, loff_t *);
static int proc_read_clk(struct seq_file *, void *);
static ssize_t proc_write_clk(struct file *, const char __user *, size_t, loff_t *);
#if defined(ENABLE_NAPI) && ENABLE_NAPI
static int proc_read_napi(struct seq_file *, void *);
static ssize_t proc_write_napi(struct file *, const char __user *, size_t, loff_t *);
#endif
static int proc_read_prio(struct seq_file *, void *);
static ssize_t proc_write_prio(struct file *, const char __user *, size_t, loff_t *);
static int proc_read_ewan(struct seq_file *, void *);
static ssize_t proc_write_ewan(struct file *, const char __user *, size_t, loff_t *);
...
static ssize_t proc_write_mirror(struct file *, const char __user *, size_t, loff_t *);
#endif

static int proc_read_class(struct seq_file *, void *);
static ssize_t proc_write_class(struct file *, const char __user *, size_t, loff_t *);
static int proc_read_mccfg(struct seq_file *, void *);
static ssize_t proc_write_mccfg(struct file *, const char __user *, size_t, loff_t *);
static int proc_read_vrx218_swap(struct seq_file *, void *);
static int proc_write_vrx218_swap(struct file *, const char __user *, size_t, loff_t *);
...
static int proc_read_vrx218_swap_seq_open(struct inode *, struct file *);


#if defined(CONFIG_ACCL_11AC)
static int proc_read_vap_mib_seq_open(struct inode *, struct file *);
static int proc_read_directlink_port_seq_open(struct inode *, struct file *);
static int proc_read_vapmib(struct seq_file *, void *);
static int proc_read_dtlk_port(struct seq_file *, void *);
#endif
...
#else
        clk = clk_get_ppe();
        tx_qos_cfg.time_tick    = clk_get_rate (clk) / 62500; //  16 * (lk_get_rate (clk)() / 1000000)
...
static struct file_operations g_proc_file_directforwarding_seq_fops = {
    ...
    .read       = seq_read,
    .write      = proc_write_directforwarding,
    .llseek     = seq_lseek,
    .release    = single_release,
};
...
static struct file_operations g_proc_file_vrx218_swap_seq_fops = {
    .owner      = THIS_MODULE,
    .open       = proc_read_vrx218_swap_seq_open,
    .read       = seq_read,
    .write      = proc_write_vrx218_swap,
    .llseek     = seq_lseek,
    .release    = single_release,
};
static int proc_read_vrx218_swap_seq_open(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_vrx218_swap, NULL);
}

#if defined(CONFIG_ACCL_11AC)
static struct file_operations g_proc_file_vap_mib_seq_fops = {
    .owner      = THIS_MODULE,
    .open       = proc_read_vap_mib_seq_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release    = single_release,
};
static int proc_read_vap_mib_seq_open(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_vapmib, NULL);
}

static struct file_operations g_proc_file_directlink_port_seq_fops = {
    .owner      = THIS_MODULE,
    .open       = proc_read_directlink_port_seq_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release    = single_release,
};
static int proc_read_directlink_port_seq_open(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_dtlk_port, NULL);
}
...
static void ppa_proc_reg(struct proc_entry_cfg *proc_entry)
{
    ...
    res  = proc_create("vrx218_swap",S_IRUGO|S_IWUSR, proc_dir, &g_proc_file_vrx218_swap_seq_fops);

#if defined(CONFIG_ACCL_11AC)
    res  = proc_create("vap_mib",S_IRUGO|S_IWUSR, proc_dir, &g_proc_file_vap_mib_seq_fops);
    res  = proc_create("directlink_port",S_IRUGO|S_IWUSR, proc_dir, &g_proc_file_directlink_port_seq_fops);
#endif
...
static INLINE void proc_file_create(void)
{
    /*if no parent folder name provided, register function will take the nearest proc_dir_entry by default.
     ...
     // Parent dir,    Name,    IS_DIR,   RD_fn,     WR_fn,     Enable
        ...
        {NULL,"vrx218_swap",		0, NULL,      NULL, 	1 },
#if defined(CONFIG_ACCL_11AC)
	{NULL,  "vap_mib",              0, NULL,      NULL,      1 },
        {NULL,  "directlink_port",      0, NULL,      NULL,      1 },
#endif
...
static ssize_t proc_write_mib(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char *p1, *p2;
    int len;
    int colon;
...
static ssize_t proc_write_route(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
...
static int proc_write_genconf(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char str[2048];
    char *p;
    int len, rlen;
...
static ssize_t proc_write_hit(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char str[2048];
    char *p;
    int len, rlen;
...
static ssize_t proc_write_burstlen(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
static ssize_t proc_write_directforwarding(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char str[2048];
    char *p;
    int len, rlen;
...
static int proc_read_clk(struct seq_file *seq, void *v)
{
    ...
    seq_printf(seq, (*AMAZON_S_CGU_SYS & 0x10000) ? "PPE clock - 250M\n" : "PPE clock - 400M\n");

   
    return 0;
}

static ssize_t proc_write_clk(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char str[2048];
    char *p;
    int len, rlen;
...
static ssize_t proc_write_napi(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char str[2048];
    char *p;
    int len, rlen;
...
static ssize_t proc_write_prio(struct file *file, const char __user  *buf, size_t count, loff_t *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
static ssize_t proc_write_ewan(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
#else
    clk = clk_get_ppe();
    seq_printf(seq, "  PPE clk=%u MHz, basic tick=%u\n", (u32)(clk_get_rate(clk) / 1000000), (u32) TX_QOS_CFG->time_tick / (clk_get_rate(clk)  / 1000000));
...
static ssize_t proc_write_mirror(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char str[2048];
    char *p;
    int len, rlen;
...
static ssize_t proc_write_class(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
static ssize_t proc_write_mccfg(struct file *file, const char __user *buf, size_t count, loff_t *data)
{
    char local_buf[2048];
    int len;
    char *p1, *p2;
...
static int proc_read_vapmib(struct seq_file *seq,void *v)
{
	int i;
	int len = 0;
	volatile mib_table_t *vap_mib = (mib_table_t *)SB_BUFFER(__D6_PER_VAP_MIB_BASE);

	for(i = 0; i < __D6_SUPPORT_VAP_NUM; i ++){
		seq_printf(seq, "VAP%d: txpdu: %u, txbytes: %u\n", i, vap_mib[i].txpdu,  vap_mib[i].txbytes); 
	}
	return len;
}

static int proc_read_dtlk_port(struct seq_file *seq,void *v)
{
    int i;
	int len = 0;

    seq_printf(seq, "directlink Port info:\n");
    for(i = 0; i < MAX_DTLK_NUM; i ++){
        if(g_ppe_dtlk_data[i].flags & PPE_DTLK_VALID){
            seq_printf(seq, "[%d]: Directpath id: %d, VAP id: %d, wifi dev: %s\n",
                i, g_ppe_dtlk_data[i].dp_if_id,g_ppe_dtlk_data[i].vap_id, g_ppe_dtlk_data[i].dev->name);
        }else{
...
static int proc_read_vapmib(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int i;
	int len = 0;
	volatile mib_table_t *vap_mib = (mib_table_t *)SB_BUFFER(__D6_PER_VAP_MIB_BASE);
	...
	for(i = 0; i < __D6_SUPPORT_VAP_NUM; i ++){
		len += sprintf(page + len, "VAP%d: txpdu: %u, txbytes: %u\n", i, vap_mib[i].txpdu,  vap_mib[i].txbytes); 
	}
	*eof = 1;
...
static int proc_read_dtlk_port(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    int i;
	int len = 0;

    len += sprintf(page + len, "directlink Port info:\n");
    for(i = 0; i < MAX_DTLK_NUM; i ++){
        if(g_ppe_dtlk_data[i].flags & PPE_DTLK_VALID){
            len += sprintf(page + len, "[%d]: Directpath id: %d, VAP id: %d, wifi dev: %s\n",
                i, g_ppe_dtlk_data[i].dp_if_id,g_ppe_dtlk_data[i].vap_id, g_ppe_dtlk_data[i].dev->name);
        }else{
        ...
        case PPA_GENERIC_DATAPATH_GET_PPE_VERION:
            {
                ...
                if ( version->index == 0 )
                    break;
                version->index -= 1;
                ret = ifx_ppe_ext_datapath_generic_hook != NULL ? ifx_ppe_ext_datapath_generic_hook(cmd, buffer, flag) : PPA_FAILURE;
                version->index += 1;
                return ret;
            }
...
{
    if ( is_atm_vcc ){
        if(ppa_drv_get_atm_qid_with_pkt_hook){
            return ppa_drv_get_atm_qid_with_pkt_hook(skb, arg, is_atm_vcc);
        }
    }
    else
    {
...
int directlink_send(uint32_t *p_if_id, struct sk_buff *skb, int32_t len, uint32_t flags)
{
	...
	if((flags & FRAME_FROM_DL) && g_ppe_dtlk_data[dl_id].dp_if_id < 0){//directlink and not directpath
            ...
            if(eth_xmit(skb, 3 /* CPU or dummy */, 3, 3 /* spid */, (int32_t)vap_id /* SW classification */, XMIT_DIRECTLINK_TO_WIFI) != PPA_SUCCESS){
...
int ppe_directpath_send(uint32_t if_id, struct sk_buff *skb, int32_t len, uint32_t flags)
{
...
#ifdef CONFIG_ACCL_11AC
    if((ret = directlink_send(&if_id,skb,len,flags)) != PPA_EAGAIN){
        return ret;
    }else{
         if(flags == FRAME_FROM_DL)
    ...
    spin_lock_irqsave(&g_directpath_tx_spinlock, sys_flag);
#if !defined(ENABLE_DIRECTPATH_TX_QUEUE) || !ENABLE_DIRECTPATH_TX_QUEUE
    if ( eth_xmit(skb, dpid /* CPU or dummy */, 3, if_id /* spid */, -1 /* SW classification */,xmit_flag) )
    {
        g_ppe_directpath_data[off].tx_pkt_dropped++;
        ret = -1;
    }
...
{
    unsigned long sys_flag;
    unsigned int ppe_cfg_bit = arg == PPA_PWM_LEVEL_D0 ? (4 << 16) : (1 << 16);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 7)
    struct clk * clk ;
#endif
...
//        .change_mtu     = eth_change_mtu,
    ...
    *MBOX_IGU1_IER = g_mailbox_signal_mask;
    ...
    /*  create proc file    */
    //ppa_drv_proc_init(g_ipv6_acc_en);
    proc_file_create();
...
int32_t ppa_directlink_enable (uint32_t flags)
{
...
int32_t ppa_directlink_disable (uint32_t flags )
{
...
uint32_t wlan_pltfrm_ppa_offload_get_txpb_base(void)
{
	uint32_t offl_vpe1_load_addr;
	uint32_t txpb;
	uint32_t wlanm = 0x100000;
	uint32_t size = 0x800000;
#if 0
        offl_vpe1_load_addr = vpe1_get_load_addr(0);
	txpb = offl_vpe1_load_addr - wlanm - size;
	return txpb;
#endif

#if 1
        char *cmd_line_buf;
        cmd_line_buf = strstr(saved_command_line,"mem=");
        if(cmd_line_buf)
        {
        	sscanf(cmd_line_buf+4,"%dM",&txpb);
        }
        else
        {
        	printk("\nError...\n");
...
uint32_t ppa_dl_qca_h2t_ring_init ( uint32_t h2tRingSize, uint32_t entrySize,uint32_t src_ring_base,uint32_t pcie_base, uint32_t flags )
{
	...
	BADR_TXPB=wlan_pltfrm_ppa_offload_get_txpb_base();
	...
	*SB_BUFFER(__DMA_RXDES_INDEX) = __D6_RXDMA7_DES_BASE;
	 ...
	 */
	BADR_HTT_TXDES = SB_BUFFER(__D6_HTT_TX_DES_BASE);
	for(i=0; i<NUM_HTT_TXDES ; i++)
	{
	  bufptr = BADR_HTT_TXDES + (i*LEN_HTT_TXDES);// 32 or 16 <by default>
	  memset(bufptr, 0x0, 4);
...
uint32_t ppa_dl_qca_cpu_h2t_ring_init (uint32_t h2tCpuMsgRingSize, uint32_t entrySize, uint32_t flags)
{
	...
	*SB_BUFFER(__CFG_SIZE_CPUSRCDES) = h2tCpuMsgRingSize;
 	...
 	return 	SB_BUFFER(__CPU_CE4_BUF_BASE);
}

int32_t ppa_dl_qca_cpu_h2t_ring_get_write_idx (uint32_t flags)
{
 	return 	SB_BUFFER(__CPU_CE4_WRITE_INDEX);
}
...
int32_t ppa_dl_qca_cpu_h2t_ring_write_msg ( uint32_t writeIndex, uint32_t * msgPtr, uint32_t msgLen, uint32_t next_writeIndex, uint32_t flags )
{
	...
	void* bufptr = SB_BUFFER(__CPU_CE4_BUF_BASE + 2*writeIndex);
...
int32_t ppa_dl_qca_cpu_txpkt_buf_release ( uint32_t num_msdus,uint32_t * msg, uint32_t flags )
{
    uint32_t bit01;
    int i;
    int counter = 0;
    int maxloopcount=256; //0x1000
...
uint32_t ppa_dl_qca_get_vap_stats ( uint32_t vapId, PPA_WLAN_VAP_Stats_t * vapStats, uint32_t flags )
{
	...
	volatile mib_table_t *vap_mib = (mib_table_t *)SB_BUFFER(__D6_PER_VAP_MIB_BASE);
    	...
    	if( (vapId == 0x0) || (vapStats == NULL))
		return PPA_FAILURE;

	//vapStats ->tx_pkts  = g_ppe_dtlk_data[vapId].dl_tx_pkts;
	vapStats ->tx_pkts  = vap_mib[vapId].txpdu;
	vapStats ->tx_bytes = vap_mib[vapId].txbytes;
	vapStats ->rx_pkts = vap_mib[vapId].rx_rcv_pdu;
	vapStats ->rx_bytes = vap_mib[vapId].rx_rcv_bytes;
	vapStats ->rx_disc_pkts =vap_mib[vapId].rx_discard_pdu;
	vapStats ->rx_disc_bytes =vap_mib[vapId].rx_discard_bytes;

	vapStats ->rx_fwd_pkts =vap_mib[vapId].rx_fwd_pdu;
	vapStats ->rx_fwd_bytes =vap_mib[vapId].rx_fwd_bytes;
	vapStats ->rx_insp_pkts =vap_mib[vapId].rx_inspect_pdu;
	vapStats ->rx_insp_bytes =vap_mib[vapId].rx_inspect_bytes;
	vapStats ->rx_pn_pkts=vap_mib[vapId].rx_pn_pdu;
	vapStats ->rx_pn_bytes=vap_mib[vapId].rx_pn_bytes;
	vapStats ->rx_drop_pkts = vap_mib[vapId].rx_drop_pdu;
	vapStats ->rx_drop_bytes=vap_mib[vapId].rx_drop_bytes;
...
EXPORT_SYMBOL(ppa_directlink_disable);
...
EXPORT_SYMBOL(ppa_dl_qca_cpu_txpkt_buf_release);
...
#if 0
uint32_t ppa_dl_qca_get_rx_mdu_stats ( PPA_WLAN_RX_MPDU_MSDU_Stats_t * rxMduStats, uint32_t flags )
{
...
int32_t ppa_dl_qca_clear_stats ( PPA_WLAN_DL_STATS_Clear_t clearType, uint32_t flags )
{

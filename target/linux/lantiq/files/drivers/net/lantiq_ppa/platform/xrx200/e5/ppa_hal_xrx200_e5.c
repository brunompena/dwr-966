...
static void add_tunnel_6rd_header(struct net_device *dev, volatile struct iphdr *iph);
    ...
    static uint32_t set_qos_port_id(void)
    {
        ...
        else if ( wan_itf & PPA_DEST_LIST_ATM ) qos_queue_portid = PPA_WAN_QOS_PTM0;
        else
        {
            qos_queue_portid = -1; //invalid
            res = PPA_FAILURE;
...
uint32_t add_dslite_tunnel_entry(struct net_device *dev, uint32_t *tunnel_idx)
{
    ...
    volatile struct ipv6hdr *ip6_hdr;
    uint32_t ret;
    struct ip6_tnl *t;
    ...
    for(entry = 0; entry < MAX_6RD_TUNNEL_ENTRIES; entry ++){
        if(!g_dslite_tunnel_counter[entry] && empty_entry >= MAX_6RD_TUNNEL_ENTRIES){
            empty_entry = entry;
        }else{
            ip6_hdr = (struct ipv6hdr *)TUNNEL_DSLITE_TBL(entry);
            if(ipv6_addr_cmp((void *)&ip6_hdr->saddr, &t->parms.laddr) == 0 && ipv6_addr_cmp((void *)&ip6_hdr->daddr, &t->parms.raddr)== 0){
                goto ADD_TUNNEL_ENTRY_GOON;
            }
        }
    }
    ...
    if(entry >= MAX_DSLITE_TUNNEL_ENTRIES){
        ...
        ipv6_addr_copy((void *)&ip6_hdr->saddr, &t->parms.laddr);
        ipv6_addr_copy((void *)&ip6_hdr->daddr, &t->parms.raddr);
    }
   ...
   case PPA_GENERIC_HAL_GET_PPE_FW_VERSION:
        {
            ...
            return ppa_drv_datapath_generic_hook(PPA_GENERIC_DATAPATH_GET_PPE_VERION, buffer, 0 );
        }

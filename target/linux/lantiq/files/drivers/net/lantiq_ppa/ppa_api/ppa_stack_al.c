...
#else
static int32_t ppa_get_6rd_dst_mac(struct net_device *dev, PPA_BUF *ppa_buf,uint8_t *mac)
#endif
{
    ...
    if ( ppa_check_is_ppp_netif(phy_dev) )
    {
        ...
        if( ppa_pppoe_get_dst_mac(phy_dev, mac) == PPA_SUCCESS){
            return PPA_SUCCESS;
        }
#else
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,8,10)
        if(ppa_get_6rd_dmac_fn != NULL && ppa_get_6rd_dmac_fn(dev, mac) == PPA_SUCCESS){
#else
        if(ppa_get_6rd_dmac_fn != NULL && ppa_get_6rd_dmac_fn(dev, ppa_buf,mac) == PPA_SUCCESS){
#endif
            return PPA_SUCCESS;
        }
     }
...
#else
static int32_t ppa_get_dslite_dst_mac(struct net_device *dev,PPA_BUF* buf, uint8_t *mac)
#endif
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,8,10)
        if(ppa_get_ip4ip6_dmac_fn != NULL && ppa_get_ip4ip6_dmac_fn(dev, mac) == PPA_SUCCESS){
#else
        if(ppa_get_ip4ip6_dmac_fn != NULL && ppa_get_ip4ip6_dmac_fn(dev,buf, mac) == PPA_SUCCESS){
#endif
            return PPA_SUCCESS;
        }
     }
...
int32_t ppa_get_dst_mac(PPA_BUF *ppa_buf, PPA_SESSION *p_session, uint8_t mac[PPA_ETH_ALEN])
{
...
#else
        return ppa_get_6rd_dst_mac(ppa_buf->dev,ppa_buf, mac);
#endif
    }
...
#else
        return ppa_get_dslite_dst_mac(ppa_buf->dev,ppa_buf, mac);
#endif
    }
    /* First need to check if PPP output interface */
    ...
    neigh = dst_neigh_lookup_skb(dst,ppa_buf);
    if(dst->dev->header_ops->cache == NULL)
    {
       //printk("\n%s,%d, no headerOps so calling neigh_hh_init & neigh_update_hhs \n",__FUNCTION__,__LINE__);
    ...
    if(neigh->nud_state & NUD_CONNECTED)
    	printk("\n%s,%d,NUD connected",__FUNCTION__,__LINE__);
    else
    	printk("\n%s,%d,NUD not connected",__FUNCTION__,__LINE__);
#endif
    ...
    if ( !hh )
    {
        if ( neigh
            && (neigh->ha[0] != 0
                ...
                || neigh->ha[5] != 0) )
        {
            ppa_memcpy(mac, (uint8_t *)neigh->ha, PPA_ETH_ALEN);
            neigh_release(neigh);
            return PPA_SUCCESS;
        }
...
int32_t ppa_get_br_dst_port(PPA_NETIF *netif, PPA_BUF *ppa_buf, PPA_NETIF **p_netif)
{
    ...
    //dest = ppa_buf->data;
    if ( (ret = ppa_get_dst_mac(ppa_buf, NULL, dest)) != PPA_SUCCESS )

...
### Merge from PPA.sh script in S17PPA.sh ###
	...
	if [ "1$CONFIG_FEATURE_PPA_SUPPORT" = "11" ]; then
		if [ -n "$CONFIG_TARGET_LANTIQ_XRX500_HAPS" ]; then
	...
	while [ $i -lt $lan_main_Count ]
		do
			...
			if [ "$iface" = "br0" ]; then
				...
				elif [ -n "$CONFIG_TARGET_LANTIQ_XRX500_HAPS" ]; then
					echo "GRX500 HAPS... lan interace init..."
					/sbin/ifconfig eth0_0 hw ether $BR_MAC_ADDR up
					sleep 1
					/usr/sbin/brctl addif $iface eth0_0
					local LAN_MAC_ADDR=`echo $BR_MAC_ADDR | /usr/sbin/next_macaddr -4`
					/sbin/ifconfig eth0_1 hw ether $LAN_MAC_ADDR up
					sleep 1
					/usr/sbin/brctl addif $iface eth0_1
					local LAN_MAC_ADDR=`echo $BR_MAC_ADDR | /usr/sbin/next_macaddr -3`
					/sbin/ifconfig eth0_2 hw ether $LAN_MAC_ADDR up
					sleep 1
					/usr/sbin/brctl addif $iface eth0_2
					LAN_MAC_ADDR=`echo $BR_MAC_ADDR | /usr/sbin/next_macaddr -2`
                                        /sbin/ifconfig eth0_3 hw ether $LAN_MAC_ADDR up
					sleep 1
					/usr/sbin/brctl addif $iface eth0_3
					LAN_MAC_ADDR=`echo $BR_MAC_ADDR | /usr/sbin/next_macaddr -1`
                                        /sbin/ifconfig eth0_4 hw ether $LAN_MAC_ADDR up
					sleep 1
	...
	#if [ -n "$CONFIG_TARGET_LANTIQ_XRX500_HAPS" ]; then
	#	/sbin/ifconfig eth0_0 0.0.0.0 up
	#else
  ...
  #fi
	...
	#				iptables -t nat -A IFX_NAPT_PREROUTING_LAN --in-interface eth0_0 --jump ACCEPT && \
	...
	[ -e /usr/local/bin/lanv6Init ]                && lanv6Init &
}

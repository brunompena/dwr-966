...
# descrypt with new key
                openssl enc -d -des -kfile $KEY_FILE -in ./$1 -out ./$TempFW &&
                {
...
# descrypt with old key
		checkFW=`cat /tmp/ap_fw_check`
		if [ "_$checkFW" = "_2" ]; then
...
# create the KEY of current branch
	...
	openssl enc -d -des -kfile ./${BRANCH}_KEY -in ./$1 -out ./$TempFW && 
	{
		echo "AP FW Check PASS!!"
		echo 0 > $CheckResult
		#do upgrade command here

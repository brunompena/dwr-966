...
if [ "$NATENABLE" = "5" ]; then 
	...
	arptables -I FORWARD -o rmnet_usb0 -s $IP_NET/$IP_MASK -j DROP
...
#Amy 20140729: add for 464XLAT
iptables -t nat -A NAT_SNAT -o clat -j MASQUERADE
if [ "$EWAN_SW" = "1" ]; then
	iptables -t nat -A NAT_SNAT -o eth1 -j MASQUERADE
	iptables -t nat -A NAT_SNAT -o ppp0 -j MASQUERADE
fi
...
if [ "_$1" = "_init" ];then
	if [ `netstat -ln | grep :67 | awk '{print $2}'` != 0 ];then
		killall -9 udhcpd
	fi
fi

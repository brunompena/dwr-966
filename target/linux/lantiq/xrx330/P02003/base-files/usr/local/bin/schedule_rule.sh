...
if [ "_$1" = "_release_URLLink" ];then
	if [ "$SchedulesEnable" = "1" ]; then
		...
		if [ "$PCProfileCnt" -ne 0 ]; then
			sqlite3 /tmp/system.db "select _ROWID_ from UrlProfile where ProfileName != ''" | while read i
			do
				PCProfileName=`sqlite3 -cmd ".timeout 1000" /tmp/system.db "select ProfileName from UrlProfile where _ROWID_='$i'";`
				iptables -F PC$PCProfileName
				iptables -Z PC$PCProfileName
			done
		fi
	fi
	return
	...
	if [ "$SchedulesEnable" = "1" ]; then
		if [ "_$2" != "_" ];then  
			...
			if [ "_$3" != "_" ];then
				...
				echo +$2 > /proc/net/xt_recent/idle_$3
			else
		...
		do
			...
			if [ "$PCProfileSchEnable" = "1" ]; then
				...
				if [ "$SchName" != "" ]; then
					...
					iptables -A SCH_RULE -m recent --name PC$PCProfileName --rcheck --seconds $SessionTimeout -m time --weekdays $WeekDays --timestart $StartHour:$StartMin --timestop $EndHour:$EndMin -j PC$PCProfileName
					...
					if [ "$PCProfileURLEnable" = "1" ]; then
						...
						iptables -A PC$PCProfileName -m recent --name idle_PC$PCProfileName --update --seconds $IdleTimeout -j CF$URLProfileName
					else	
						iptables -A PC$PCProfileName -m recent --name idle_PC$PCProfileName --update --seconds $IdleTimeout -j ACCEPT
					fi
				else	
				...
				SessionTimeout=`sqlite3 -cmd ".timeout 1000" /tmp/system.db "select SessionTimeout from UrlProfile where _ROWID_='$i'";`
				SessionTimeout=`expr $SessionTimeout \* 60`
				iptables -A SCH_RULE  -m recent --name PC$PCProfileName --rcheck --seconds $SessionTimeout -j PC$PCProfileName
